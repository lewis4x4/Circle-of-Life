...
function FacilityPackets({ facilityId }: { facilityId: string }) {
  ...
  return <div className="space-y-6">
    ...
    {data && <>
      ...
      {showSettings && data.canConfigure && <PolicyForm key={`${facilityId}-${data.policy?.revision ?? 0}`} facilityId={facilityId} policy={data.policy} facilities={data.facilities} onSaved={() => setRefresh((n) => n + 1)} />}
