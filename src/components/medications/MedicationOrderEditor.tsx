...
export function MedicationOrderEditor({ residentId, medicationId, onSaved }: {
    ...
    return <section className="space-y-3 rounded border p-4"><fieldset disabled={busy} className="contents"><h3 className="font-semibold">{medicationId ? "Medication order revision" : "Record a medication order"}</h3><p>Transcribe the authorized prescriber order. A revision preserves the prior order and its administrations.</p>{["medication_name", "strength", "form", "indication", "prescriber_name", "order_date", "start_date", "end_date", "scheduled_times", "instructions", "prn_max_frequency"].map(key => <label className="block" key={key}>{enumLabel(key, { case: "lower" })}{key === "scheduled_times" ? " (HH:MM, comma-separated)" : ""}<input type={key.endsWith("date") ? "date" : "text"} value={fields[key] ?? ""} onChange={e => setFields(prior => ({ ...prior, [key]: e.target.value }))} className="block w-full rounded border p-2"/></label>)}{([["route", routes], ["frequency", frequencies], ["controlled_schedule", ["non_controlled", "ii", "iii", "iv", "v"]]] as const).map(([key, options]) => <label className="block" key={key}>{enumLabel(key, { case: "lower" })}<select value={fields[key] ?? ""} onChange={e => setFields(prior => ({ ...prior, [key]: e.target.value }))} className="block w-full rounded border p-2"><option value="">Choose</option>{options.map(value => <option key={value} value={value}>{enumLabel(value, { case: "lower" })}</option>)}</select></label>)}<label className="block">Order evidence / reason for change<textarea value={reason} onChange={e => setReason(e.target.value)} className="block w-full rounded border p-2"/></label>{error && <p role="alert">{error}</p>}<div className="flex gap-3"><Button disabled={busy || !reason.trim() || required.some(key => !fields[key]?.trim())} onClick={() => void save("save")}>Save authorized order</Button>{medicationId && <Button variant="outline" disabled={busy || !reason.trim()} onClick={() => void save("discontinue")}>Discontinue per order</Button>}</div></fieldset></section>;
}
