...
function DetailContent({ detail }: {
    ...
    const { batch } = detail;
    return <div className="space-y-5"><p>{statusLabels[batch.status]}. Accounting unverified; business release and external posting disabled.</p>{detail.invalid_reason ? <p>Current blocker: {enumLabel(detail.invalid_reason, { case: "lower" })}</p> : null}
