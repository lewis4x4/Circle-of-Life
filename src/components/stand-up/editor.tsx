...
export function StandUpEditor(props: Props) {
  ...
  return <section aria-label={`${facility.name} entry`} className="space-y-5">
    ...
    <section aria-label="Save and submit report" className="sticky bottom-0 z-10 space-y-2 border-y border-border bg-background px-1 py-2 shadow-sm sm:space-y-3 sm:py-4">
      ...
      <a href={`/print/stand-up?facility=${encodeURIComponent(facility.id)}&week=${encodeURIComponent(week)}`} target="_blank" rel="noopener" className="inline-block text-sm font-medium underline underline-offset-2">Print for the binder<span className="sr-only"> (opens a printable copy of {facility.name}, {dateLabel(week)})</span></a>
