...
import { METRICS, metricDisplay, validateValues, type MetricKey, type StandUpReport, type StandUpValues } from '@/lib/stand-up/model';
...
export function RecoveryTools({ facility, week, report, recoveries, disabled, onSaved, onBusy, onDenied, onReload }: {
  ...
  return <fieldset disabled={busy || disabled || routePending} className="space-y-4 rounded border border-border p-4">
    <h3 className="font-semibold">Spreadsheet changes · {facility.name}</h3><p className="text-sm text-muted-foreground">During a Haven outage, use the shared Stand Up workbook while Drive is available. Changes needing a decision appear here. Review them before replacing any saved figures.</p>
