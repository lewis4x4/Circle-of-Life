...
import { METRICS, METRIC_KEYS, emptyValues, metricDisplay, dateLabel, shiftDay, staffingPeriod, type MetricKey, type StandUpValues } from '@/lib/stand-up/model';
...
export function EntryQuestions({ fields, onChange, disabled, week, prior, priorWeek }: {
  fields: EntryFields; onChange: (key: keyof EntryFields, value: string) => void; disabled: boolean;
  week: string; prior?: StandUpValues; priorWeek?: string;
}) {
      ...
      <div className={`grid gap-x-5 gap-y-4 ${section === 'Admissions' || section === 'Marketing' ? 'sm:grid-cols-2 lg:grid-cols-3' : 'sm:grid-cols-2'}`}>
        {METRICS.filter(metric => metric.section === section).map(metric => metric.key === 'overtime_reported' ? <div key={metric.key} className="space-y-2">
          <p id="overtime-label" className="text-sm font-medium">Overtime last week</p><div role="group" aria-labelledby="overtime-label" className="grid max-w-sm grid-cols-2 gap-3">
            <label className="text-xs text-muted-foreground">Hours<Input aria-label="Overtime hours" type="number" min="0" step="1" inputMode="numeric" value={fields.overtime_hours} onChange={e => onChange('overtime_hours', e.target.value)} className="mt-1 tabular-nums" /></label>
            <label className="text-xs text-muted-foreground">Minutes<Input aria-label="Overtime minutes" type="number" min="0" max="59" step="1" inputMode="numeric" value={fields.overtime_minutes} onChange={e => onChange('overtime_minutes', e.target.value)} className="mt-1 tabular-nums" /></label>
          </div><p className="text-xs text-muted-foreground">Enter hours and minutes. For example: 17 hours, 15 minutes.</p>{prior && <p className="text-xs text-muted-foreground">{priorWeek}: {metricDisplay(metric.key, prior[metric.key])}</p>}
          ...
          {prior && <span className="block text-xs text-muted-foreground">{priorWeek}: {metricDisplay(metric.key, prior[metric.key])}</span>}
        </label>)}
      </div>
    </fieldset>;
  })}</div>;
