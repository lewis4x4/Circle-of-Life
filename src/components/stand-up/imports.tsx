...
export function HistoricalImports({ onReload, onDenied, bindGuard }: {
  ...
  return <fieldset disabled={routePending || busy} aria-label="Historical import tools" className="mt-3 space-y-4 rounded border border-border p-4">
    ...
    <details className="border-t border-border pt-3"><summary className="cursor-pointer text-sm font-medium">Reverse an imported batch</summary><div className="mt-3 space-y-3"><p className="text-sm">A reversal preserves the original history and later independent edits. Any conflicts still require review.</p><label className="block text-sm">Import batch ID<Input value={batchId} disabled={busy} onChange={event => setBatchId(event.target.value)} /></label><label className="block text-sm">Reason for reversal<Input value={reason} disabled={busy} onChange={event => setReason(event.target.value)} /></label><Button variant="outline" disabled={busy || !batchId.trim() || !reason.trim()} onClick={() => void run(async () => { const result = await standUpRequest<{ restored: unknown[]; conflicts: unknown[] }>('reverse_import', { batch_id: batchId, reason }); if (!alive.current) return; setNotice(`Reversal completed. ${Array.isArray(result.restored) ? result.restored.length : String(result.restored)} restored; ${Array.isArray(result.conflicts) ? result.conflicts.length : String(result.conflicts)} require review.`); await onReload(); })}>Reverse import and retain history</Button></div></details>
  </fieldset>;
}
