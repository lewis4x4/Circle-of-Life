...
// Below the fold and hand-off panels load after the page shell (COL-703): they
// are rendered on the server as usual, but their code is a separate chunk, not
// part of /admin/executive's first-load JavaScript.
const belowFold = () => import("@/components/executive/ExecutiveOverviewBelowFold");
const PortfolioComparisonTable = dynamic(() => belowFold().then((m) => m.PortfolioComparisonTable));
const PortfolioFiguresStrip = dynamic(() => belowFold().then((m) => m.PortfolioFiguresStrip));
const RoundingAssuranceTable = dynamic(() => belowFold().then((m) => m.RoundingAssuranceTable));
const ResidentPresenceBand = dynamic(() => belowFold().then((m) => m.ResidentPresenceBand));
const SupportingDestinations = dynamic(() => belowFold().then((m) => m.SupportingDestinations));
const EscalatedFromFacilitiesPanel = dynamic(() =>
  import("@/components/executive/EscalatedFromFacilitiesPanel").then((m) => m.EscalatedFromFacilitiesPanel),
);
const CensusNoticesPanel = dynamic(() =>
  import("@/components/executive/CensusNoticesPanel").then((m) => m.CensusNoticesPanel),
);
const CollectionEscalationsPanel = dynamic(() =>
  import("@/components/executive/CollectionEscalationsPanel").then((m) => m.CollectionEscalationsPanel),
);
