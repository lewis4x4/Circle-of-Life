...
function RowBadges({ row }: { row: BoardRow }) {
  ...
  return (
    <div className="flex flex-wrap gap-1">
      ...
      {row.documents_expiring != null && row.documents_expiring > 0 && <StatusPill tone="warning">{row.documents_expiring === 1 ? "Document expiring" : `${row.documents_expiring} documents expiring`}</StatusPill>}
