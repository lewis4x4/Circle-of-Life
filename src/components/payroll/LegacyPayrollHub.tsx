    ...
    <div className="relative w-full space-y-6 pb-12">
      ...
      <div className="relative z-10 space-y-6">
        <header className="mb-8">
          <Link href="/admin/payroll" className="text-sm text-primary underline">Payroll packets</Link>
          <div>

            <h2 className="text-3xl font-semibold tracking-tight text-foreground flex items-center gap-3">
              Legacy payroll batches
            </h2>
          </div>
        </header>
        <p className="text-sm text-muted-foreground">These historical exports use legacy time records. New payroll packets use reviewed kiosk timecards or payroll-reviewed hours.</p>
          ...
          <div className="h-[160px]">
            <V2Card hoverColor="emerald" className="border-emerald-500/20 shadow-[inset_0_0_15px_rgba(16,185,129,0.05)]">
              ...
              <div className="relative z-10 flex flex-col h-full justify-between">
                ...
                {batchCount.status === "value" ? (
                  <p className="text-4xl tracking-tighter text-success pb-1 tabular-nums">
                    {batchCount.value}
                  </p>
                ) : (
              ...
              {displayRows.map((row) => (
                <MotionItem key={row.id}>
                  <TableRow render={<Link href={`/admin/payroll/${row.id}`} />}>
                    <span className="flex-[2] min-w-0 text-[12px] text-foreground tabular-nums truncate">
                    ...
                    <span className="w-[120px] shrink-0 text-[11px] text-muted-foreground tabular-nums text-right">
