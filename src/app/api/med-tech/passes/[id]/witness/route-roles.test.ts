...
beforeEach(() => {
  vi.clearAllMocks(); originRole = "med_tech"; witnessRole = "med_tech";
  ...
  mocks.rpc.mockImplementation(() => ["med_tech", "caregiver"].includes(witnessRole) ? { data: "signature", error: null } : { data: null, error: { message: "Invalid clinical witness" } });
});
const witness = () => POST(new NextRequest("http://local/witness", { method: "POST", body: JSON.stringify({ email: "witness@example.test", password: "temporary-test-input" }) }), { params: Promise.resolve({ id: "pass" }) });
it("lets the actual med-tech pass owner request a med-tech witness", async () => {
...
it("does not let a legacy nurse role originate a witnessed pass", async () => {
  originRole = "nurse"; expect((await witness()).status).toBe(403);
});
