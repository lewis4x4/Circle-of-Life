...
import { RECEIPT_COMMAND_ROLES, isReceiptOutcome, mapReceiptRpcError, payloadProblem, withoutRequestHash, verifyWorkBodySchema } from "@/lib/operations/receipts";
 ...
 * receipt for occurrences whose rule requires review. The database enforces
 * the reviewer role, independence from the performer and recorder, evidence
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!parsed.success) {
    return NextResponse.json({ error: payloadProblem(parsed.error) ?? "Provide a request key and a verification decision", outcome: "validation" }, { status: 400 });
  }
