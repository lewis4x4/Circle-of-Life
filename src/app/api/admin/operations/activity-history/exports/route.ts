...
// request_id is a client-generated idempotency token, not an existing entity ID.
const schema = z.object({ facility_id: databaseUuidSchema, activity_id: databaseUuidSchema, request_id: z.string().uuid() }).strict();
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!result || !databaseUuidSchema.safeParse(result.export_id).success || !validManifest(result.manifest)) throw new HistoryExportError("Export manifest unavailable; retry the same request");
