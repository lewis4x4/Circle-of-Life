...
export default async function AssignedEmployeeReviews() {
  ...
  if (grants.error) return <div className="p-6"><p role="alert">Reviewer access could not be verified. Please sign in again.</p></div>;
  ...
  return <div className="space-y-4 p-6"><Link className="text-sm underline" href="/employee-file">My employee file</Link><h1 className="text-2xl font-semibold">Assigned medical-file reviews</h1>{result.error ? <p role="alert">Employee records could not be loaded.</p> : !staff.length ? <p>No employee files are available under your current reviewer access.</p> : <ul className="space-y-3">{staff.map((s) => <li key={s.id}><Link className="underline" href={`/employee-file/reviews/${s.id}`}>{s.first_name} {s.last_name} · {s.facility_name ?? "Assigned community"}</Link></li>)}</ul>}</div>;
}
