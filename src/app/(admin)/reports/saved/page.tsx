      ...
      {canManage && (
        <div className="p-6 sm:p-8 rounded-lg border border-slate-200/60 dark:border-white/5 bg-slate-50/50 shadow-sm relative overflow-visible mb-6 z-10 w-full transition-all">
          ...
          <div className="grid gap-4 flex-col lg:flex-row lg:grid-cols-[2fr_1fr_auto] items-center">
            ...
            <Button className="rounded-2xl font-mono uppercase tracking-wider text-[11px] font-bold h-12 w-full lg:w-auto hover:-translate-y-0.5 transition-transform shadow-lg px-10" onClick={() => void onCreate()}>
              Save Variant
            </Button>
          </div>
        </div>
                  ...
                  <MotionItem key={view.id}>
                    <div className="p-6 rounded-lg group transition-all duration-300 hover:scale-[1.01] cursor-default border border-slate-200 dark:border-white/5 bg-white/80 w-full flex flex-col xl:flex-row xl:items-center justify-between gap-6 shadow-sm hover:shadow-md hover:border-slate-300 dark:hover:border-white/20">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-6 w-full items-center">
                           ...
                           <div className="flex flex-col gap-2 align-left md:text-left">
                              ...
                              {view.pinned_template_version ? (
                                ...
                                <Badge className="bg-emerald-50 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400 border-emerald-200 dark:border-emerald-500/20 uppercase tracking-wider font-mono text-[9px] font-bold shadow-sm px-2.5 py-1 rounded-full w-fit">
                                  Live Updates
                                </Badge>
                              )}
                           </div>
                        ...
                        <div className="flex shrink-0 xl:ml-4 gap-3 mt-4 xl:mt-0">
                           <Link href={`/admin/reports/run/saved_view/${view.id}`} className={cn(buttonVariants({ variant: "outline", size: "sm" }), "font-mono uppercase tracking-wider text-[10px] h-10 rounded-xl font-bold bg-white dark:bg-white/5 border-slate-200 dark:border-white/10 w-full lg:w-auto px-6 hover:bg-slate-50 dark:hover:bg-white/10 shadow-sm transition-colors")}>
                             Run Report
                           </Link>
                           <Button variant="ghost" size="sm" onClick={() => void onArchive(view.id)} className="font-mono uppercase tracking-wider text-[10px] h-10 rounded-xl font-bold border-transparent hover:bg-rose-50 hover:text-rose-600 dark:hover:bg-rose-500/10 dark:hover:text-rose-400 px-6 w-full lg:w-auto transition-colors">
                             Archive
                           </Button>
                        </div>
                    </div>
