      ...
      <div className="flex gap-2">
        {(["active", "discontinued", "all"] as const).map((t) => (
          <button
            ...
            onClick={() => setTab(t)}
            className={cn(
              "rounded-[1rem] px-6 py-2.5 text-[10px] font-bold uppercase tracking-wider transition-all",
              tab === t
            ...
            {filtered.length === 0 ? (
              ...
              <>
                 <div className="hidden lg:grid grid-cols-[1.5fr_1fr_2fr_1fr_1fr] gap-4 px-6 pb-4 border-b border-slate-200 dark:border-white/5 relative z-10 text-left">
                   <div className="text-[10px] font-bold uppercase tracking-wider text-slate-500 dark:text-zinc-500">Medication</div>
                   <div className="text-[10px] font-bold uppercase tracking-wider text-slate-500 dark:text-zinc-500">Route</div>
                   <div className="text-[10px] font-bold uppercase tracking-wider text-slate-500 dark:text-zinc-500">Frequency & Schedule</div>
                   <div className="text-[10px] font-bold uppercase tracking-wider text-slate-500 dark:text-zinc-500">Prescriber</div>
                   <div className="text-[10px] font-bold uppercase tracking-wider text-slate-500 dark:text-zinc-500">Dates</div>
                 </div>
                       ...
                       <MotionItem key={m.id}>
                         <div className="grid grid-cols-1 lg:grid-cols-[1.5fr_1fr_2fr_1fr_1fr] gap-4 lg:items-center p-6 rounded-lg bg-white border border-slate-100 dark:border-white/5 shadow-sm tap-responsive group hover:border-indigo-200 dark:hover:border-indigo-500/30 hover:shadow-lg dark:hover:bg-white/[0.05] transition-all duration-300 w-full outline-none">
                           ...
                           <div className="flex flex-col">
                             <span className="lg:hidden text-xs text-slate-500 uppercase tracking-wider font-bold mb-1">Route</span>
                             <span className="text-sm font-medium capitalize text-slate-800 dark:text-slate-300">{m.route.replace(/_/g, " ")}</span>
                           </div>

                           <div className="flex flex-col">
                             <span className="lg:hidden text-xs text-slate-500 uppercase tracking-wider font-bold mb-1">Frequency & Schedule</span>
                             ...
                             {m.scheduled_times?.length ? (
                                <div className="flex flex-wrap gap-1 mt-1.5">
                                   {m.scheduled_times.map(t => (
                                     <span key={t} className="px-1.5 py-0.5 bg-slate-100 dark:bg-slate-800 rounded text-[9px] font-mono tracking-wider text-slate-600 dark:text-slate-400">{t}</span>
                                   ))}
                                </div>
                             ) : null}
                           </div>
                           ...
                           <div className="flex flex-col">
                             <span className="lg:hidden text-xs text-slate-500 uppercase tracking-wider font-bold mb-1">Prescriber</span>
                             <span className="text-xs font-medium text-slate-600 dark:text-slate-400">{m.prescriber_name ?? "—"}</span>
                           </div>

                           <div className="flex flex-col lg:items-end">
                             <span className="lg:hidden text-xs text-slate-500 uppercase tracking-wider font-bold mb-1">Dates</span>
                             <span className="text-[11px] font-mono tracking-wider text-slate-500 dark:text-zinc-500">START</span>
                             <span className="text-sm font-semibold text-slate-800 dark:text-slate-200 mt-0.5">{m.start_date}</span>
                             {m.status !== "active" && (
                                <Badge className="mt-2 bg-slate-200 text-slate-700 dark:bg-slate-800 dark:text-slate-400 border-none shadow-none uppercase font-bold text-[9px] tracking-wider">{m.status}</Badge>
                             )}
                           </div>
