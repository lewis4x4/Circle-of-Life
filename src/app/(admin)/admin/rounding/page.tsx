...
// Treat `missed` and `excused` as terminal — they remove a task from the
// in-flight queue so the "completed" strip state can be reached when the
// shift has misses to review.
...
export default function AdminRoundingHubPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const taskData = (tasksRes.data ?? []) as TaskRow[];
      ...
      const pendingApprovals = watchRows.filter(
        (row) => row.status === "pending_approval",
      ).length;

      if (expected === 0) {
        setEmptyNotice("no_rounds");
      }
