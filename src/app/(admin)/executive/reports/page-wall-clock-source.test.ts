...
describe("executive saved reports CSV export Eastern date stamp", () => {
  ...
  it("names CSV exports with the Eastern calendar date after 8pm ET", () => {
    ...
    expect(`haven-exec-${safe}-${dateStamp}.csv`).toBe("haven-exec-Weekly_ops-2026-08-20.csv");
  });
  ...
  it("uses todayFacilityDateIso for CSV filenames and labels exports as Eastern", () => {
    ...
    expect(pageSource).toContain("`haven-exec-${safe}-${todayFacilityDateIso()}.csv`");
    expect(pageSource).toContain("CSV export filenames use today&apos;s Eastern (ET) calendar date.");
    expect(pageSource).not.toMatch(/haven-exec-\$\{safe\}-\$\{new Date\(\)\.toISOString\(\)\.slice\(0,\s*10\)\}/);
    expect(pageSource).toContain("generated_at");
    expect(pageSource).toContain("new Date().toISOString()");
    expect(pageSource).toContain("last_generated_at: new Date().toISOString()");
