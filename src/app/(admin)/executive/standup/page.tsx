      ...
      <div className="relative z-10 space-y-6">
              ...
              return (
                <Card key={metricKey as string} className="rounded-lg border border-slate-200/70 bg-white/70 shadow-sm dark:border-white/10 dark:bg-white/[0.03]">
                  <CardHeader className="pb-3">
                    <CardDescription className="text-[10px] font-bold uppercase tracking-[0.18em] text-muted-foreground">
                      {metricLabel}
                    </CardDescription>
                  ...
                  <CardContent className="pt-0">
                    ...
                    {standupMetricNote(metric) ? (
                      <p className="mt-2 text-xs text-muted-foreground">{standupMetricNote(metric)}</p>
                    ) : null}
                  </CardContent>
                </Card>
              );
              ...
              : facilityCards.map((facility) => (
                  <Card key={facility.facilityId} className="rounded-lg border border-slate-200/70 bg-white/70 shadow-sm dark:border-white/10 dark:bg-white/[0.03]">
                    ...
                    <CardContent className="space-y-2 text-sm text-slate-600 dark:text-zinc-300">
                      ...
                      <div className="pt-2">
                        <div className="text-[10px] font-bold uppercase tracking-[0.18em] text-muted-foreground">What is flagged</div>
        ...
        <section className="grid grid-cols-1 gap-4 lg:grid-cols-[1.1fr_0.9fr]">
          <Card className="rounded-lg border border-slate-200/70 bg-white/70 shadow-sm dark:border-white/10 dark:bg-white/[0.03]">
            ...
            <CardContent className="space-y-4">
              {leadActions.length === 0 ? (
                <div className="text-sm text-muted-foreground">No intervention recommendations yet.</div>
              ) : (
          ...
          <Card className="rounded-lg border border-slate-200/70 bg-white/70 shadow-sm dark:border-white/10 dark:bg-white/[0.03]">
            ...
            <CardContent className="space-y-4">
              {leadActions.length === 0 ? (
                <div className="text-sm text-muted-foreground">No variance comparison available yet.</div>
              ) : (
                leadActions.map((action) => (
        ...
        <section className="space-y-4">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">Metrics with manual or forecast source modes are modeled but not yet fully automated.</p>
          </div>
                    ...
                    <thead>
                      <tr className="border-b border-slate-200 dark:border-white/10">
                        ...
                        {(live?.facilities ?? []).map((facility) => (
                          <th key={`${section.sectionKey}-${facility.facilityName}`} className="px-3 py-2 text-left font-semibold text-muted-foreground">
                            ...
                            return (
                              <td key={`${facility.facilityName}-${definition.key}`} className="px-3 py-3 align-top">
                                {metric ? (
                                  ...
                                  <span className="text-muted-foreground">{formatStandupMetricValue(undefined, definition.label)}</span>
                                )}
                              </td>
                            );
                          })}
