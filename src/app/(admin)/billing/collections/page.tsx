        ...
        <header className="mb-8 flex flex-col gap-6 md:flex-row md:items-end justify-between bg-emerald-50/20 p-8 rounded-lg border border-emerald-200/50 dark:border-white/5 shadow-sm mt-4">
          ...
          <div className="flex shrink-0 items-center justify-end">
            <Link
              ...
              className={cn(buttonVariants({ size: "lg" }), "rounded-full font-bold uppercase tracking-wider text-[10px] shadow-lg bg-emerald-600 hover:bg-emerald-700 text-white border border-emerald-500")}
            >
              + Log Activity
            </Link>
          </div>
        ...
        {loading ? (
          ...
          <div className="p-6 sm:p-8 rounded-lg border border-slate-200/60 dark:border-white/5 bg-slate-50/50 shadow-sm relative overflow-hidden transition-all">
            <div className="mb-6 border-b border-slate-200 dark:border-white/5 pb-4 flex items-center justify-between">
              <h3 className="text-xl font-semibold text-slate-900 dark:text-white mt-1">Activity Log</h3>
              <p className="text-[10px] font-mono tracking-wider text-slate-400 mt-1 uppercase">
                 Scoped to selected facility
              </p>
            </div>
            
            <div className="relative z-10 hidden md:grid grid-cols-12 gap-4 pb-2 mb-2 px-6 text-[10px] font-bold uppercase tracking-wider text-slate-400">
            ...
            <div className="relative z-10">
               <MotionList className="space-y-3">
                 {rows.map((r) => {
                   ...
                   return (
                     <MotionItem key={r.id}>
                       <Link href={`/admin/residents/${r.resident_id}/billing`} className="group flex flex-col md:grid md:grid-cols-12 gap-4 md:items-center p-5 rounded-lg border border-slate-200/90 bg-white dark:border-white/5 shadow-sm transform-gpu transition-all hover:border-emerald-300 dark:hover:border-emerald-500/40 hover:shadow-md outline-none">
                          <div className="col-span-2">
                            <span className="md:hidden font-bold uppercase tracking-wider text-[9px] text-slate-400 mb-1 block">Date</span>
                            <span className="text-sm font-semibold text-slate-900 dark:text-slate-100">{r.activity_date}</span>
                          </div>
                          
                          <div className="col-span-2">
                            <span className="md:hidden font-bold uppercase tracking-wider text-[9px] text-slate-400 mb-1 block">Resident</span>
                          ...
                          <div className="col-span-2">
                             <span className="md:hidden font-bold uppercase tracking-wider text-[9px] text-slate-400 mb-1 block">Type</span>
                             <span className="inline-flex text-xs font-mono tracking-wider uppercase bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-300 px-2.5 py-1 rounded-full border border-slate-200 dark:border-slate-700">
                               {r.activity_type.replace(/_/g, " ")}
                             </span>
                          </div>

                          <div className="col-span-4 max-w-sm">
                            <span className="md:hidden font-bold uppercase tracking-wider text-[9px] text-slate-400 mb-1 block">Summary</span>
                          ...
                          <div className="col-span-2 flex items-center justify-between md:justify-end gap-4 min-w-0">
                            <div className="flex flex-col items-start md:items-end min-w-0">
                              <span className="md:hidden font-bold uppercase tracking-wider text-[9px] text-slate-400 mb-1 block text-right">Follow-up</span>
                              {r.follow_up_date ? (
                                <span className="inline-flex items-center gap-1.5 text-xs font-mono font-bold tracking-wider text-emerald-700 bg-emerald-50 border border-emerald-200 dark:border-emerald-500/20 dark:bg-emerald-500/10 dark:text-emerald-400 px-2.5 py-1 rounded-full">
