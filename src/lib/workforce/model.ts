...
export type WorkforcePerson = { id: string; name: string; role: string; facilityNames?: string[]; status: string; since: string | null; currentShift: string | null; nextShift: string | null; scheduleId: string | null; attendance: "expected" | "missing" | "extra" | "off" | "called_out" | "unknown"; scheduledMinutes: number | null; scheduleMissing?: boolean; workedMinutes: number | null; timeIncomplete?: boolean; exceptions: number; fileStatus: string; due: { title: string; date: string }[] };
export type WorkforceSnapshot = { facilityId: string | null; facilityName: string; allFacilities?: boolean; generatedAt: string; timeclockEnabled: boolean; weekStart: string; weekEnd: string; nextWeekStart: string; scheduleStatus: string; people: WorkforcePerson[]; payrollStatus: string; payrollRulesConfigured: boolean };
