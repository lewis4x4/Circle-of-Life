...
describe("qualityHubMetricValue", () => {
  it("names the facility gap before loading or counts", () => {
    expect(qualityHubMetricValue(4, { noFacility: true, loading: false })).toBe(
      qualityHubMetricNoFacilityCopy(),
    );
  });

  it("names a loading gap before showing counts", () => {
    expect(qualityHubMetricValue(4, { noFacility: false, loading: true })).toBe(
      qualityHubMetricLoadingCopy(),
    );
  });

  it("keeps real zeros once loaded", () => {
    expect(qualityHubMetricValue(0, { noFacility: false, loading: false })).toBe(0);
  });

  it("never returns an em dash", () => {
    const ctxs = [
      { noFacility: true, loading: false },
      { noFacility: false, loading: true },
      { noFacility: false, loading: false },
    ];
