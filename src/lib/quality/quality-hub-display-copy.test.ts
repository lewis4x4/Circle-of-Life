...
describe("qualityHubMetricValue", () => {
  it("names a loading gap before showing counts", () => {
    expect(qualityHubMetricValue(4, { noOrganization: false, loading: true })).toBe(
      qualityHubMetricLoadingCopy(),
    );
  });
  ...
  it("names the organization gap before showing zero counts", () => {
    expect(
      qualityHubMetricValue(0, { noOrganization: true, loading: false }),
  ...
  it("keeps real zeros once loaded", () => {
    expect(qualityHubMetricValue(0, { noOrganization: false, loading: false })).toBe(0);
  });

  it("never returns an em dash", () => {
    const ctxs = [
      { noOrganization: true, loading: false },
      { noOrganization: false, loading: true },
      { noOrganization: false, loading: false },
    ];
