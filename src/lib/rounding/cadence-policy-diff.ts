...
export function policyText(value: unknown): string {
  ...
  if (typeof value === "object") return Object.entries(value).filter(([key]) => !METADATA.has(key)).map(([key, child]) => `${enumLabel(key, { case: "lower" })}: ${policyText(child)}`).join(", ");
  return enumLabel(String(value), { case: "lower" });
}
export function policyDiff(before: Record<string, unknown>[], after: Record<string, unknown>[]) {
  ...
  return [...new Set([...old.keys(), ...next.keys()])].flatMap((key) => {
    ...
    return [...new Set([...Object.keys(left ?? {}), ...Object.keys(right ?? {})])].filter((field) => !METADATA.has(field) && JSON.stringify(stable(left?.[field])) !== JSON.stringify(stable(right?.[field]))).map((field) => ({ key: `${key}:${field}`, label: `${String(right?.label ?? left?.label ?? key)} · ${enumLabel(field, { case: "lower" })}`, before: left ? policyText(left[field]) : "Not present", after: right ? policyText(right[field]) : "Removed" }));
  });
}
