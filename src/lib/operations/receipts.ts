...
const TRUSTED_FRAGMENTS = [
  ...
  // COL-147 source links: only RAISE wordings; Postgres constraint names stay hidden.
  "not allowlisted",
  "not a candidate",
  ...
  "no longer matches the occurrence",
];
...
const VALIDATION_FRAGMENTS = ["is required", "are required", "requires", "must be", "not editable", "is invalid", "carries no identifier", "Recorded values", "Performed time", "Corrected performed time", "Performer", "not allowlisted", "not a candidate", "applies to select only", "does not match the caller", "is not a staff observation", "is recorded against", "Asset is not current", "A correction must restate", "invalid drill value", "Drill type cannot change",
  // COL-159
  "is recorded against the site", "is recorded against a named asset", "is not linked to this site", "is recorded on its service date", "is recorded as performed", "cannot change; void the record", "next_due_on", "is not a current document", "must be empty", "must name the staff performer", "no longer matches the occurrence"];
