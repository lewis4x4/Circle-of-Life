...
export async function currentRetryOwner(organizationId: string, facilityId: string): Promise<RetryOwner> {
  ...
  if (error || !actor?.user_id || !actor.session_id || actor.organization_id !== organizationId) {
    throw new Error("Your sign-in could not be confirmed. Tap Switch and unlock again.");
  }
...
export async function claimFloorCheck(taskId: string, owner: RetryOwner): Promise<void> {
  ...
  if (!response.ok || result.ok !== true) throw new Error(result.error ?? "The check could not be taken. Try again.");
}
...
export async function saveFloorCheck(input: {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return { status: "failed", message: json.error ?? FAILED_COPY };
    }
    if (json.ok !== true) return { status: "failed", message: FAILED_COPY };
    return { status: "saved" };
  } catch (error) {
    ...
    return { status: "failed", message: error instanceof Error ? error.message : FAILED_COPY };
  }
}
