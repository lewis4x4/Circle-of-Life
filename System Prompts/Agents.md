...
**Circle of Life / Haven — portable copy.** Canonical source: repo root `AGENTS.md`.
...
Broad **COL / Haven** ALF multi-entity SaaS roadmap (modules, phases, tech direction) is summarized in **`docs/roadmap-overview.md`**. Use that file for orientation only.

**Authoritative build scope:** Markdown (and related) specs you place in **`docs/specs/`** (see **`docs/specs/README.md`**). Implement from those files plus explicit user instructions—not from a generic industry roadmap alone.

## Domain execution rules (Haven)

- Treat **care operations data** as high-consequence: favor **auditability**, **least privilege**, and **clear role boundaries** (staff, clinical, family, admin).
- Prefer **verification in repo** (migrations, RLS policies, app guards) over assumptions from documents.
- **Do not** implement out-of-scope modules from orientation docs unless backed by a spec in **`docs/specs/`** or direct instruction.
- **Do not** copy sensitive real resident or workforce data into prompts, commits, or public artifacts.
