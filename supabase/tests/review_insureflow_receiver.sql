...
-- COL-514: public.insurance_documents belongs to the insurance workspace build
-- (migration 336) which was NOT carried across with the receiver. The boundary
-- assertion against it is kept and runs in full wherever that table exists; it
-- is skipped, and only skipped, when the table is absent. Referencing a missing
-- table statically aborts the whole block, which would silently drop the three
-- assertions that can still be made.
DO $$DECLARE org uuid;leaked boolean;BEGIN
 SELECT organization_id INTO org FROM receiver_probe;
 leaked := EXISTS(SELECT 1 FROM public.insurance_policies WHERE organization_id=org)
        OR EXISTS(SELECT 1 FROM public.insurance_claims WHERE organization_id=org)
        OR EXISTS(SELECT 1 FROM public.audit_log WHERE table_name LIKE 'insureflow_receiver%');
 IF NOT leaked AND to_regclass('public.insurance_documents') IS NOT NULL THEN
  EXECUTE 'SELECT EXISTS(SELECT 1 FROM public.insurance_documents WHERE organization_id=$1)' INTO leaked USING org;
 END IF;
 IF leaked THEN RAISE EXCEPTION 'Receiver crossed core or raw audit boundary';END IF;
END$$;
