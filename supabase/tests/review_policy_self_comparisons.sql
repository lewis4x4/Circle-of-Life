...
-- punch policy now refuses another organization's or building's staff id while
-- still allowing float staff assigned to the building.
BEGIN;
...
SELECT gen_random_uuid() admin_user, gen_random_uuid() admin_session,
       ...
       gen_random_uuid() own_staff, gen_random_uuid() elsewhere_staff, gen_random_uuid() foreign_staff, gen_random_uuid() float_staff
...
-- The administrator reaches both buildings, so every refusal below is the
-- policy's building check, not staff visibility.
INSERT INTO public.user_facility_access(user_id,facility_id,organization_id)
  SELECT admin_user, facility, org FROM pc UNION ALL SELECT admin_user, other_facility, org FROM pc;
INSERT INTO public.staff(id,facility_id,organization_id,first_name,last_name,staff_role,hire_date)
  ...
  UNION ALL SELECT float_staff, other_facility, org, 'Float', 'Probe', 'cna'::public.staff_role, current_date - 30 FROM pc;
-- A float staff member whose home is the annex but who also works at the house.
INSERT INTO public.staff_facility_assignments(organization_id, staff_id, facility_id)
  SELECT org, float_staff, facility FROM pc;
...
DO $$ DECLARE f pc; BEGIN
  ...
  -- ...and for float staff assigned to the building.
  INSERT INTO public.time_records(staff_id,facility_id,organization_id,clock_in,clock_in_method,approved)
    VALUES (f.float_staff, f.facility, f.org, now(), 'manual', false);
  BEGIN
    INSERT INTO public.time_records(staff_id,facility_id,organization_id,clock_in,clock_in_method,approved)
