...
BEGIN;
-- Clinical dates are Eastern even when the replay host runs in UTC.
CREATE FUNCTION pg_temp.clinical_date() RETURNS date LANGUAGE sql STABLE AS $$ SELECT (now() AT TIME ZONE 'America/New_York')::date $$;
...
DO $$ DECLARE f record; revision uuid:=gen_random_uuid(); order_data jsonb; plan_id uuid:=gen_random_uuid(); new_plan uuid:=gen_random_uuid(); BEGIN
 SELECT * INTO f FROM clinical_fixture;
 order_data:=jsonb_build_object('medication_name','Clinical fixture medication','strength','10 mg','route','oral','frequency','daily','scheduled_times',jsonb_build_array('08:00'),'instructions','One tablet per authorized order','prescriber_name','Fixture prescriber','start_date',pg_temp.clinical_date(),'order_date',pg_temp.clinical_date(),'controlled_schedule','non_controlled','form','tablet','end_date',pg_temp.clinical_date()+10,'indication','Fixture indication','prn_effectiveness_check_minutes',45);
 ...
 IF NOT EXISTS(SELECT 1 FROM resident_medications WHERE id=revision AND end_date=pg_temp.clinical_date()+10 AND form='tablet' AND indication='Fixture indication' AND prn_effectiveness_check_minutes=45) THEN RAISE EXCEPTION 'Medication revision erased clinical order fields'; END IF;
 ...
 PERFORM create_care_plan_revision_review(plan_id,f.resident,NULL,pg_temp.clinical_date(),pg_temp.clinical_date()+30,'Initial plan','[{"category":"bathing","title":"Bathing support","description":"Provide safe shower support","assistance_level":"supervision","frequency":"daily","goal":"Safe bathing","interventions":["Offer supervision"],"special_instructions":"Resident preference"}]');
 PERFORM create_care_plan_revision_review(plan_id,f.resident,NULL,pg_temp.clinical_date(),pg_temp.clinical_date()+30,'Initial plan','[{"category":"bathing","title":"Bathing support","description":"Provide safe shower support","assistance_level":"supervision","frequency":"daily","goal":"Safe bathing","interventions":["Offer supervision"],"special_instructions":"Resident preference"}]');
 UPDATE care_plans SET status='active',approved_by=f.actor,approved_at=now() WHERE id=plan_id;
 PERFORM create_care_plan_revision_review(new_plan,f.resident,plan_id,pg_temp.clinical_date(),pg_temp.clinical_date()+30,'Updated need','[{"category":"bathing","title":"Bathing support","description":"Provide safe shower support","assistance_level":"limited_assist","interventions":[]}]');
...
INSERT INTO med_passes(id,organization_id,facility_id,shift_id,resident_id,resident_medication_id,scheduled_time,administered_by)
 SELECT pass_id,org,facility,shift_id,resident,(SELECT id FROM resident_medications WHERE previous_medication_id=f.med LIMIT 1),(pg_temp.clinical_date()::text||' 08:00:00 America/New_York')::timestamptz,actor FROM clinical_fixture f;
...
INSERT INTO operation_task_instances(id,organization_id,facility_id,template_name,template_category,template_cadence_type,assigned_shift_date,assigned_to,requires_dual_sign)
 SELECT task_id,org,facility,'Clinical dual task','safety','daily',pg_temp.clinical_date(),actor,true FROM clinical_fixture;
DO $$ DECLARE f record; BEGIN
 SELECT * INTO f FROM clinical_fixture;
...
INSERT INTO emergency_checklist_items(id,facility_id,organization_id,checklist_type,title,frequency_days,next_due_date)
 SELECT checklist_id,facility,org,'generator_test','Clinical fixture checklist',30,pg_temp.clinical_date() FROM clinical_fixture;
...
DO $$ DECLARE f record; receipt uuid:=gen_random_uuid(); maintenance_id uuid:=gen_random_uuid(); payload jsonb; BEGIN
 ...
 PERFORM complete_emergency_checklist_review(receipt,f.checklist_id,ARRAY['Fixture staff'],'Recorded evidence');
 IF (SELECT count(*) FROM emergency_checklist_completions WHERE checklist_item_id=f.checklist_id)<>1 OR NOT EXISTS(SELECT 1 FROM emergency_checklist_items WHERE id=f.checklist_id AND last_completed_at IS NOT NULL AND next_due_date>pg_temp.clinical_date()) THEN RAISE EXCEPTION 'Checklist completion did not atomically advance due date'; END IF;
...
DO $$ DECLARE f record; first_id uuid:=gen_random_uuid(); second_id uuid:=gen_random_uuid(); outbreak uuid; BEGIN
 SELECT * INTO f FROM clinical_fixture;
 INSERT INTO infection_surveillance(id,resident_id,facility_id,organization_id,infection_type,onset_date,identified_by,symptoms) VALUES(first_id,f.resident,f.facility,f.org,'gi',pg_temp.clinical_date(),f.actor,ARRAY['fixture symptom']),(second_id,f.resident2,f.facility,f.org,'gi',pg_temp.clinical_date(),f.actor,ARRAY['fixture symptom']);
...
DO $$ DECLARE f record; med_id uuid; scheduled timestamptz; pending_id uuid:=gen_random_uuid(); pass2 uuid:=gen_random_uuid(); BEGIN
 ...
 scheduled:=((pg_temp.clinical_date()-1)::text||' 08:00:00 America/New_York')::timestamptz;
 ...
 BEGIN PERFORM create_care_plan_revision_review(gen_random_uuid(),f.resident,NULL,pg_temp.clinical_date(),pg_temp.clinical_date()+30,'Null plan',NULL); RAISE EXCEPTION 'Null plan items accepted'; EXCEPTION WHEN raise_exception THEN IF SQLERRM<>'Effective date, review date and care needs are required' THEN RAISE; END IF; END;
...
DO $$ DECLARE f record; admission uuid; fixture_bed uuid:=gen_random_uuid(); room uuid; BEGIN
 ...
 INSERT INTO form_1823_records(admission_case_id,resident_id,facility_id,organization_id,status,physician_name,exam_date,expiration_date,updated_at) VALUES(admission,f.resident2,f.facility,f.org,'received','Fixture physician',pg_temp.clinical_date(),pg_temp.clinical_date()+365,now()+interval '1 second');
 ...
 PERFORM confirm_admission_arrival_review(admission,f.actor,pg_temp.clinical_date());
