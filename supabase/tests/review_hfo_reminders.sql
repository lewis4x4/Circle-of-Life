-- COL-152 fixture: issue lifecycle on the disposable replay. Proves that an issue is
...
CREATE FUNCTION pg_temp.i_assert(ok boolean,msg text) RETURNS void LANGUAGE plpgsql AS $$ BEGIN IF ok IS NOT TRUE THEN RAISE EXCEPTION 'COL-152 fixture %',msg; END IF; END $$;
CREATE FUNCTION pg_temp.i_denied(stmt text) RETURNS void LANGUAGE plpgsql AS $$ BEGIN
 ...
 RAISE EXCEPTION 'COL-152 fixture expected authority denial: %',stmt;
END $$;
CREATE FUNCTION pg_temp.i_expect(stmt text,fragment text) RETURNS void LANGUAGE plpgsql AS $$ BEGIN
 ...
 RAISE EXCEPTION 'COL-152 fixture expected rejection containing "%": %',fragment,stmt;
END $$;
...
-- Freeze synthetic occurrence timing without activating any live schedule. The
-- replay fixture alone bypasses immutable occurrence guards for past-time cases.
SELECT pg_temp.i_clear();
ALTER TABLE public.operation_task_instances DISABLE TRIGGER USER;
UPDATE public.operation_task_instances SET due_at=clock_timestamp()-interval '1 hour',remind_at=clock_timestamp()-interval '2 hours',assigned_to=(SELECT maint FROM ifx) WHERE id=pg_temp.iid('occ_a1_d1');
UPDATE public.operation_task_instances SET due_at=clock_timestamp()-interval '1 hour',remind_at=NULL,assigned_to=(SELECT maint FROM ifx) WHERE id=pg_temp.iid('occ_a1_d2');
ALTER TABLE public.operation_task_instances ENABLE TRIGGER USER;
-- A reader-only manager and a maintenance recipient are ineligible for a
-- resident response; neither may block routing to a current nurse backup.
SELECT pg_temp.i_login('admin_a');
SELECT pg_temp.i_assert(NOT haven.operation_reminder_recipient_current((SELECT mgr FROM ifx),pg_temp.iid('occ_res_d1')),'resident reader without recording authority became recipient');
SELECT pg_temp.i_assert(NOT haven.operation_reminder_recipient_current((SELECT maint FROM ifx),pg_temp.iid('occ_res_d1')),'maintenance role became resident recipient');
SELECT pg_temp.i_assert(haven.operation_reminder_recipient_current((SELECT nurse FROM ifx),pg_temp.iid('occ_res_d1')),'current nurse recipient rejected');
SELECT pg_temp.i_login('maint'); SET LOCAL ROLE authenticated;
INSERT INTO ifx_results VALUES('reminder',public.operation_reminder_review(pg_temp.iid('occ_a1_d1'),'refresh'));
SELECT pg_temp.i_assert((SELECT result->>'state'='active' AND result->>'phase'='overdue' AND result->>'delivery_status'='queued' FROM ifx_results WHERE label='reminder'),'approved overdue episode');
INSERT INTO ifx_results VALUES('again',public.operation_reminder_review(pg_temp.iid('occ_a1_d1'),'refresh'));
SELECT pg_temp.i_assert((SELECT a.result=b.result FROM ifx_results a JOIN ifx_results b ON b.label='again' WHERE a.label='reminder'),'duplicate refresh changed episode');
SELECT pg_temp.i_assert((public.operation_reminder_review(pg_temp.iid('occ_a1_d2'),'refresh')->>'state')='configuration_needed','unknown reminder window became overdue alert');
INSERT INTO ifx_results SELECT 'ack',public.operation_reminder_review(pg_temp.iid('occ_a1_d1'),'acknowledge',(result->>'revision')::uuid,NULL,'ack-request-0001') FROM ifx_results WHERE label='reminder';
SELECT pg_temp.i_assert((SELECT result->>'suppressed'='true' AND result->>'delivery_status'='sent' FROM ifx_results WHERE label='ack'),'ack did not suppress');
SELECT pg_temp.i_assert((SELECT status='pending' FROM public.operation_task_instances WHERE id=pg_temp.iid('occ_a1_d1')),'ack completed the work');
INSERT INTO ifx_payload VALUES('snooze',jsonb_build_object('until',clock_timestamp()+interval '100 milliseconds'));
INSERT INTO ifx_results SELECT 'snooze',public.operation_reminder_review(pg_temp.iid('occ_a1_d1'),'snooze',(r.result->>'revision')::uuid,(p.payload->>'until')::timestamptz,'snooze-request-0001') FROM ifx_results r,ifx_payload p WHERE r.label='ack' AND p.label='snooze';
SELECT pg_sleep(0.15);
INSERT INTO ifx_results SELECT 'snooze-retry',public.operation_reminder_review(pg_temp.iid('occ_a1_d1'),'snooze',(r.result->>'revision')::uuid,(p.payload->>'until')::timestamptz,'snooze-request-0001') FROM ifx_results r,ifx_payload p WHERE r.label='ack' AND p.label='snooze';
SELECT pg_temp.i_assert((SELECT result->>'replayed'='true' AND result->>'suppressed'='false' FROM ifx_results WHERE label='snooze-retry'),'expired successful snooze retry did not replay');
SELECT pg_temp.i_assert((public.operation_reminder_review(pg_temp.iid('occ_a1_d1'),'acknowledge',(SELECT (result->>'revision')::uuid FROM ifx_results WHERE label='reminder'),NULL,'ack-request-0001')->>'replayed')='true','older request lost after newer response');
-- Two independent open problems on the same task keep separate windows/ownership.
INSERT INTO ifx_results VALUES('problem1',public.report_operation_issue_review('reminder-problem-0001',jsonb_build_object('task_instance_id',pg_temp.iid('occ_a1_d1'),'kind','problem','summary','Fixture first problem')));
INSERT INTO ifx_results VALUES('problem2',public.report_operation_issue_review('reminder-problem-0002',jsonb_build_object('task_instance_id',pg_temp.iid('occ_a1_d1'),'kind','problem','summary','Fixture second problem')));
INSERT INTO ifx_ids SELECT label,(result->'issue'->>'id')::uuid FROM ifx_results WHERE label IN('problem1','problem2');
RESET ROLE; SELECT pg_temp.i_login('admin_a'); SET LOCAL ROLE authenticated;
SELECT public.assign_operation_issue_review(pg_temp.iid('problem1'),'reminder-assign-0001',pg_temp.rev('problem1'),jsonb_build_object('owner_user_id',(SELECT maint FROM ifx),'backup_user_id',(SELECT admin_a FROM ifx)));
SELECT public.assign_operation_issue_review(pg_temp.iid('problem2'),'reminder-assign-0002',pg_temp.rev('problem2'),jsonb_build_object('owner_user_id',(SELECT admin_a FROM ifx)));
SELECT public.wait_operation_issue_review(pg_temp.iid('problem1'),'reminder-wait-0001',pg_temp.rev('problem1'),jsonb_build_object('reason','Fixture follow up','follow_up_at',clock_timestamp()+interval '100 milliseconds'));
SELECT public.wait_operation_issue_review(pg_temp.iid('problem2'),'reminder-wait-0002',pg_temp.rev('problem2'),jsonb_build_object('reason','Fixture independent follow up','follow_up_at',clock_timestamp()+interval '100 milliseconds'));
SELECT pg_sleep(0.15);
INSERT INTO ifx_results VALUES('issue1',public.operation_reminder_review(pg_temp.iid('occ_a1_d1'),'refresh',NULL,NULL,NULL,pg_temp.iid('problem1')));
INSERT INTO ifx_results VALUES('issue2',public.operation_reminder_review(pg_temp.iid('occ_a1_d1'),'refresh',NULL,NULL,NULL,pg_temp.iid('problem2')));
SELECT pg_temp.i_assert((SELECT a.result->>'id'<>b.result->>'id' AND a.result->>'state'='active' AND b.result->>'state'='active' FROM ifx_results a JOIN ifx_results b ON b.label='issue2' WHERE a.label='issue1'),'two known issue windows collapsed');
SELECT pg_temp.i_denied(format('SELECT public.operation_reminder_review(%L,%L,%L,NULL,%L,%L)',pg_temp.iid('occ_a1_d1'),'acknowledge',(SELECT result->>'revision' FROM ifx_results WHERE label='issue1'),'wrong-actor-request',pg_temp.iid('problem1')));
RESET ROLE; SELECT pg_temp.i_login('maint'); SET LOCAL ROLE authenticated;
INSERT INTO ifx_results SELECT 'issue1ack',public.operation_reminder_review(pg_temp.iid('occ_a1_d1'),'acknowledge',(result->>'revision')::uuid,NULL,'issue-ack-0001',pg_temp.iid('problem1')) FROM ifx_results WHERE label='issue1';
-- Completion resolves the task episode, never its independent problem episodes.
SELECT public.record_operation_work_review(pg_temp.iid('occ_a1_d1'),'reminder-complete-0001','{"outcome":"performed"}');
SELECT pg_temp.i_assert((public.operation_reminder_review(pg_temp.iid('occ_a1_d1'),'refresh')->>'state')='resolved','completed source did not resolve task reminder');
SELECT pg_temp.i_assert((public.operation_reminder_review(pg_temp.iid('occ_a1_d1'),'refresh',NULL,NULL,NULL,pg_temp.iid('problem1'))->>'state')='active','task completion resolved independent issue');
SELECT public.resolve_operation_issue_review(pg_temp.iid('problem1'),'reminder-resolve-0001',pg_temp.rev('problem1'),'{"resolution_summary":"Fixture fixed"}');
SELECT pg_temp.i_assert((public.operation_reminder_review(pg_temp.iid('occ_a1_d1'),'refresh',NULL,NULL,NULL,pg_temp.iid('problem1'))->>'state')='resolved','resolved issue did not resolve episode');
SELECT public.reopen_operation_issue_review(pg_temp.iid('problem1'),'reminder-reopen-0001',pg_temp.rev('problem1'),'{"reason":"Fixture recurred"}');
INSERT INTO ifx_results VALUES('reopened',public.operation_reminder_review(pg_temp.iid('occ_a1_d1'),'refresh',NULL,NULL,NULL,pg_temp.iid('problem1')));
SELECT pg_temp.i_assert((SELECT result->>'generation'='2' AND result->>'acknowledged_at' IS NULL AND result->>'state'='configuration_needed' FROM ifx_results WHERE label='reopened'),'reopen inherited suppression or fabricated window');
RESET ROLE; SELECT pg_temp.i_login('owner'); SET LOCAL ROLE authenticated;
SELECT pg_temp.i_assert(NOT EXISTS(SELECT 1 FROM public.audit_log WHERE table_name IN('operation_reminder_responses','operation_escalation_deliveries')),'generic audit leaked reminder responses');
RESET ROLE; SELECT pg_temp.i_clear();
SELECT pg_temp.i_assert((SELECT count(*)=1 FROM public.operation_escalation_deliveries WHERE task_instance_id=pg_temp.iid('occ_a1_d1') AND reminder_issue_id IS NULL),'task duplicate suppression failed');
SELECT pg_temp.i_assert((SELECT count(*)=2 FROM public.operation_escalation_deliveries WHERE task_instance_id=pg_temp.iid('occ_a1_d1') AND reminder_issue_id IS NOT NULL),'issue episode identity failed');
SELECT pg_temp.i_assert((SELECT count(*)=3 FROM public.operation_reminder_responses),'replay wrote duplicate command receipt');
-- Revoked current recipient routes only to explicit valid backup.
...
SELECT public.wait_operation_issue_review(pg_temp.iid('problem1'),'reminder-wait-0003',pg_temp.rev('problem1'),jsonb_build_object('reason','Fixture changed window','follow_up_at',clock_timestamp()+interval '100 milliseconds'));
SELECT pg_sleep(0.15);
SELECT pg_temp.i_assert((public.operation_reminder_review(pg_temp.iid('occ_a1_d1'),'refresh',NULL,NULL,NULL,pg_temp.iid('problem1'))->>'can_respond')='true','explicit valid backup was not selected');
RESET ROLE; SELECT pg_temp.i_login('maint'); SET LOCAL ROLE authenticated;
SELECT pg_temp.i_denied(format('SELECT public.operation_reminder_review(%L,%L)',pg_temp.iid('occ_a1_d1'),'refresh'));
RESET ROLE; SELECT pg_temp.i_service(); SET LOCAL ROLE service_role;
SELECT pg_temp.i_denied(format('SELECT public.operation_reminder_review(%L,%L)',pg_temp.iid('occ_a1_d1'),'refresh'));
SELECT pg_temp.i_denied(format('INSERT INTO public.operation_escalation_deliveries(organization_id,facility_id,task_instance_id,channel,delivery_status) SELECT organization_id,facility_id,id,%L,%L FROM public.operation_task_instances WHERE id=%L','sms','sent',pg_temp.iid('occ_a1_d1')));
RESET ROLE;
ROLLBACK;
