...
-- Later HFO restrictive policies hide every generic payload below, even from owners.
INSERT INTO public.audit_log(table_name,record_id,action,new_data,organization_id,facility_id,created_at)
 SELECT t,gen_random_uuid(),'INSERT',jsonb_build_object('secret','LATER-HFO-SECRET-'||t),org,site_a,'2098-06-01 12:00:00+00'::timestamptz
 FROM x CROSS JOIN unnest(ARRAY['operation_execution_receipts','operation_issues','operation_activity_bindings','operation_occurrence_associations','operation_issue_events','operation_evidence','operation_evidence_events','operation_command_drafts','operation_source_events','operation_source_event_attempts','operation_source_adapters','operation_source_rules','operation_source_record_requests','operation_reminder_responses','operation_escalation_deliveries']) t;
