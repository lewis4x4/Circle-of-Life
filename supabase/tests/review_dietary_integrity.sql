...
INSERT INTO auth.users(id,email,raw_app_meta_data,raw_user_meta_data) SELECT actor,actor||'@dietary-review.invalid',jsonb_build_object('organization_id',org,'app_role','cook'),jsonb_build_object('full_name','Dietary test') FROM dietary_fixture;
INSERT INTO public.user_profiles(id,organization_id,email,full_name,app_role,is_active) SELECT actor,org,actor||'@dietary-review.invalid','Dietary test','cook',true FROM dietary_fixture
...
SELECT set_config('request.jwt.claims',jsonb_build_object('sub',f.actor,'session_id',f.actor_session,
  ...
  'role','authenticated','app_role','cook','organization_id',f.org,
  'app_metadata',jsonb_build_object('app_role','cook','organization_id',f.org))::text,true)
