-- 2026-09-22 (Brian): nurse is not used; med-tech takes nurse's place (migration 468).
...
END $$;
RESET ROLE;
DO $$ DECLARE f record; tech uuid; tech2 uuid; caregiver uuid; manager uuid; owner_id uuid; count_id uuid; BEGIN
 SELECT * INTO f FROM count_role_fixture;
 ...
 -- One person cannot witness their own count.
 BEGIN
  PERFORM public.complete_verified_controlled_counts(ARRAY[count_id],tech2,tech2,f.facility,f.org);
  RAISE EXCEPTION 'Self-witnessed count was improperly authorized';
 EXCEPTION WHEN raise_exception THEN IF SQLERRM<>'Two different staff and at least one count are required' THEN RAISE; END IF; END;
 -- An owner is not a clinical witness.
 BEGIN
  PERFORM public.complete_verified_controlled_counts(ARRAY[count_id],tech2,owner_id,f.facility,f.org);
  RAISE EXCEPTION 'Owner witness was improperly authorized';
 EXCEPTION WHEN raise_exception THEN IF SQLERRM<>'Invalid witness' THEN RAISE; END IF; END;
 PERFORM public.complete_verified_controlled_counts(ARRAY[count_id],tech2,caregiver,f.facility,f.org);
 SELECT id INTO count_id FROM public.controlled_substance_counts WHERE outgoing_staff_id=tech;
 PERFORM public.complete_verified_controlled_counts(ARRAY[count_id],tech,tech2,f.facility,f.org);
 IF NOT EXISTS(SELECT 1 FROM public.controlled_substance_counts WHERE id=count_id AND incoming_staff_id=tech2 AND incoming_signed_at IS NOT NULL) THEN RAISE EXCEPTION 'Med-tech count could not be completed by a second med-tech'; END IF;
 INSERT INTO public.controlled_substance_counts(resident_medication_id,facility_id,organization_id,count_date,shift,expected_count,actual_count,outgoing_staff_id)
  VALUES(f.med,f.facility,f.org,current_date,'day',2,2,manager) RETURNING id INTO count_id;
 BEGIN
  PERFORM public.complete_verified_controlled_counts(ARRAY[count_id],manager,tech2,f.facility,f.org);
...
DO $$ DECLARE f record; a record; tech uuid; tech2 uuid; owner_id uuid; shift_id uuid; pass_id uuid; resident uuid; signature uuid; BEGIN
 SELECT * INTO f FROM count_role_fixture;
 SELECT id INTO tech FROM count_test_actors WHERE label='med_tech';
 SELECT id INTO tech2 FROM count_test_actors WHERE label='med_tech_2';
 SELECT id INTO owner_id FROM count_test_actors WHERE label='owner';
 SELECT resident_id INTO resident FROM public.resident_medications WHERE id=f.med;
 FOR a IN SELECT * FROM count_test_actors WHERE label IN('med_tech','med_tech_2') LOOP
  ...
  IF a.label='med_tech_2' THEN
    -- An owner cannot witness a medication pass.
    BEGIN
      PERFORM public.record_verified_med_pass_witness(pass_id,tech2,owner_id);
      RAISE EXCEPTION 'Owner pass witness was improperly authorized';
    EXCEPTION WHEN raise_exception THEN IF SQLERRM<>'Both staff must have clinical facility access' THEN RAISE; END IF; END;
    signature:=public.record_verified_med_pass_witness(pass_id,tech2,tech);
    IF NOT EXISTS(SELECT 1 FROM public.witness_signatures WHERE id=signature AND witness_user_id=tech) THEN RAISE EXCEPTION 'Second med-tech witness not saved'; END IF;
  ELSE
    signature:=public.record_verified_med_pass_witness(pass_id,tech,tech2);
    IF NOT EXISTS(SELECT 1 FROM public.witness_signatures WHERE id=signature AND med_pass_id=pass_id AND witness_user_id=tech2) THEN RAISE EXCEPTION 'Med-tech-owned pass did not save med-tech witness'; END IF;
  END IF;
 END LOOP;
END $$;
ROLLBACK;
