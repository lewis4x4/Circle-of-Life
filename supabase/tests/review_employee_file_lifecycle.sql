...
SELECT pg_temp.employee_expect(format('SELECT public.haven_employee_file_command(%L,''submit_record'',%L)',(SELECT employee FROM employee_fixture),jsonb_build_object('requirement_id',(SELECT value->>'id' FROM employee_receipts WHERE name='draft'),'completed_on',CURRENT_DATE)),'Approved applicable requirement');
...
SELECT pg_temp.employee_actor(true);
SELECT pg_temp.employee_expect(format('SELECT public.haven_employee_file_command(%L,''submit_record'',%L)',(SELECT employee FROM employee_fixture),jsonb_build_object('requirement_id',(SELECT value->>'id' FROM employee_receipts WHERE name='requirement'),'completed_on',CURRENT_DATE+1)),'Completion cannot');
INSERT INTO employee_receipts VALUES('record',public.haven_employee_file_command((SELECT employee FROM employee_fixture),'submit_record',jsonb_build_object('requirement_id',(SELECT value->>'id' FROM employee_receipts WHERE name='requirement'),'completed_on',CURRENT_DATE,'notes','Immutable evidence')));
...
INSERT INTO employee_receipts VALUES('purpose_record',public.haven_employee_file_command((SELECT employee FROM employee_fixture),'submit_record',jsonb_build_object('requirement_id',(SELECT value->>'id' FROM employee_receipts WHERE name='purpose_requirement'),'completed_on',CURRENT_DATE)));
SELECT pg_temp.employee_actor(NULL);
