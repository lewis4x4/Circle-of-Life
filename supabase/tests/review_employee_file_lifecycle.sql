...
BEGIN;
-- Employee completion dates follow the facility calendar, not the CI host clock.
CREATE FUNCTION pg_temp.employee_date() RETURNS date LANGUAGE sql STABLE AS $$ SELECT (now() AT TIME ZONE 'America/New_York')::date $$;
...
INSERT INTO public.staff(id,organization_id,facility_id,user_id,first_name,last_name,staff_role,hire_date) SELECT employee,org,facility,worker,'File','Employee','resident_aide'::public.staff_role,pg_temp.employee_date()-100 FROM employee_fixture UNION ALL SELECT admin_staff,org,facility,admin,'File','Manager','administrator'::public.staff_role,pg_temp.employee_date()-100 FROM employee_fixture;
...
INSERT INTO public.staff(id,organization_id,facility_id,first_name,last_name,staff_role,hire_date)
 SELECT other_employee,org,other_facility,'Other','Employee','resident_aide',pg_temp.employee_date()-100 FROM employee_fixture;
...
SELECT pg_temp.employee_expect(format('SELECT public.haven_employee_file_command(%L,''submit_record'',%L)',(SELECT employee FROM employee_fixture),jsonb_build_object('requirement_id',(SELECT value->>'id' FROM employee_receipts WHERE name='draft'),'completed_on',pg_temp.employee_date())),'Approved applicable requirement');
...
SELECT pg_temp.employee_actor(true);
SELECT pg_temp.employee_expect(format('SELECT public.haven_employee_file_command(%L,''submit_record'',%L)',(SELECT employee FROM employee_fixture),jsonb_build_object('requirement_id',(SELECT value->>'id' FROM employee_receipts WHERE name='requirement'),'completed_on',pg_temp.employee_date()+1)),'Completion cannot');
INSERT INTO employee_receipts VALUES('record',public.haven_employee_file_command((SELECT employee FROM employee_fixture),'submit_record',jsonb_build_object('requirement_id',(SELECT value->>'id' FROM employee_receipts WHERE name='requirement'),'completed_on',pg_temp.employee_date(),'notes','Immutable evidence')));
...
INSERT INTO employee_receipts VALUES('purpose_record',public.haven_employee_file_command((SELECT employee FROM employee_fixture),'submit_record',jsonb_build_object('requirement_id',(SELECT value->>'id' FROM employee_receipts WHERE name='purpose_requirement'),'completed_on',pg_temp.employee_date())));
SELECT pg_temp.employee_actor(NULL);
