...
Migration `486_workforce_roster_publisher.sql` creates `haven.workforce_publisher_state` and `haven.workforce_publisher_receipts`. Both have RLS and no direct grants to anonymous, member or service roles. Service-only security-definer RPCs independently require the service JWT role and a live lease token/run/generation for every sensitive step.
