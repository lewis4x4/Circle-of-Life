...
import { addDays, type LegacyAssetSchedule, legacyTemplateRule, listOccurrenceDates, localDateOf, MAX_ENUMERATION_DAYS, parseDateOnly, resolveOccurrence, SCHEDULE_EVALUATOR_VERSION, validateScheduleRule } from "../../../src/lib/operations/schedule-evaluator.ts";
...
type CandidateInstance = {
  ...
  requires_dual_sign: boolean;
  due_at: string;
  /** COL-133 classification stamped by the service job so a signed-in operator can read the row. */
  subject_id?: string;
  authority_class?: "facility" | "asset";
};

type AssetScheduleRow = LegacyAssetSchedule & { id: string; facility_id: string };
type SubjectRow = { id: string; facility_id: string; subject_kind: string; asset_id: string | null };
...
Deno.serve(async (req) => {
  ...
  // COL-568: an asset with its own self-test clock overrides the template's
  // weekday and shift-derived deadline (the generator: Tue 10:00 from the asset,
  // reset by a vendor visit). Read once for every linked asset.
  const assetRefs = Array.from(new Set(templates.map((template) => template.asset_ref).filter((id): id is string => Boolean(id))));
  const assetScheduleById = new Map<string, AssetScheduleRow>();
  if (assetRefs.length > 0) {
    const { data: assetData, error: assetError } = await admin
      .from("facility_assets")
      .select("id, facility_id, run_check_weekday, run_check_local_time")
      .in("id", assetRefs)
      .is("deleted_at", null)
      .neq("status", "retired");
    if (assetError) {
      t.log({ event: "asset_query_failed", outcome: "error", error_message: assetError.message });
      return jsonResponse({ error: "Failed to load asset schedules" }, 500, origin);
    }
    for (const row of (assetData ?? []) as AssetScheduleRow[]) assetScheduleById.set(row.id, row);
  }

  // COL-133: a legacy row is invisible to every signed-in operator until it
  // carries a current subject classification. The service job may stamp the
  // facility or asset subject (never a person); rows for a building with no
  // subject on file stay unclassified exactly as before.
  const facilitySubjectByFacility = new Map<string, string>();
  const assetSubjectByAsset = new Map<string, SubjectRow>();
  {
    const { data: subjectData, error: subjectError } = await admin
      .from("operation_activity_subjects")
      .select("id, facility_id, subject_kind, asset_id")
      .in("facility_id", facilityIds)
      .in("subject_kind", ["facility", "asset"]);
    if (subjectError) {
      t.log({ event: "subject_query_failed", outcome: "error", error_message: subjectError.message });
      return jsonResponse({ error: "Failed to load activity subjects" }, 500, origin);
    }
    for (const row of (subjectData ?? []) as SubjectRow[]) {
      if (row.subject_kind === "facility") facilitySubjectByFacility.set(row.facility_id, row.id);
      else if (row.subject_kind === "asset" && row.asset_id) assetSubjectByAsset.set(row.asset_id, row);
    }
  }
  ...
  for (const facility of facilities) {
    ...
    for (const template of facilityTemplates) {
      ...
      const assetSchedule = template.asset_ref ? assetScheduleById.get(template.asset_ref) ?? null : null;
      const assetSubject = template.asset_ref ? assetSubjectByAsset.get(template.asset_ref) : undefined;
      const classification: Pick<CandidateInstance, "subject_id" | "authority_class"> =
        assetSubject && assetSubject.facility_id === facility.id
          ? { subject_id: assetSubject.id, authority_class: "asset" }
          : facilitySubjectByFacility.has(facility.id)
            ? { subject_id: facilitySubjectByFacility.get(facility.id), authority_class: "facility" }
            : {};
      for (const shift of expandTemplateShifts(template)) {
        const translated = legacyTemplateRule(template, shift, facilityTimezone, assetSchedule);
