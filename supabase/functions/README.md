...
| `officer-catalog` | no | `POST` `{ "op": "catalog" }` or `{ "op": "execute", ... }` — Front Office capability federation target (`front-office-capability-v1`, target `haven`). Auth: **`x-fo-key-id` / `x-fo-sent-at` / `x-fo-nonce` / `x-fo-signature`** HMAC-SHA256 over the raw body, verified before parsing; then `public.officer_catalog` / `public.officer_execute` (service_role-only doors into schema `officer`, migration **`339`**). Aggregate-only reads (`occupied_beds`, `occupancy_rate`, `ar_open_balance`, `billed_revenue_mtd`, `open_incidents`, each by facility slug or `all`) plus the synthetic `ping` command. Secret named by `officer.gateway_keys.secret_env` (**`OFFICER_GATEWAY_HMAC_FRONT_OFFICE_V1`**). Ships with the key disabled. See `docs/specs/OFFICER-CAPABILITY-CATALOG.md`. |
