BEGIN;
-- COL-326 forward correction: an obsolete expectation revision is refused at once.
-- Preserve migrations 371 and 372, the existing function grants and ownership, and
-- every immutable historical request, version, event and capture. The signature is
-- unchanged, so 371's REVOKE/GRANT on this function continue to apply.
...
BEGIN
 ...
 -- COL-326 fast refusal for an obsolete expectation revision.
 -- The authoritative check below runs only after this command has taken the
 -- operation authority row lock, the source-family table locks and the
 -- per-expectation advisory lock. A request carrying a stale revision had to
 -- win every one of those before being told it was doomed, so a conflict was
 -- observed as a multi-minute wait instead of a refusal.
 -- This unlocked pre-read never accepts anything: a revision that passes here
 -- is still re-checked under the advisory lock, and a change racing between
 -- the two is refused there exactly as before. It is gated on current manage
 -- authority so an actor who could not act on this expectation still sees the
 -- same answer it received before, and expectation existence stays unrevealed.
 IF p_action NOT IN('register','capture_meeting') AND p_payload ? 'expectation_id' AND p_payload ? 'expected_revision' THEN
  SELECT * INTO e FROM haven.corporate_submission_expectations WHERE id=(p_payload->>'expectation_id')::uuid;
  IF FOUND AND (p_payload->>'expected_revision')::uuid IS DISTINCT FROM haven.corporate_expectation_revision(e.id)
     AND coalesce((haven.corporate_deliverable_context(p_task,e.period_start,e.period_end)->>'can_manage')::boolean,false) THEN
