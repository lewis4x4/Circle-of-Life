-- COL-546: turn the InsureFlow receiver on.
--
-- Migration 448 locked the receiver to synthetic mode in nine places, on purpose:
-- its independent review authorised no live connection. Brian ruled the five
-- remaining activation decisions on 2026-09-21, so this opens exactly what those
-- rulings cover and nothing more.
--
-- The two function bodies below are derived from 448 by mechanical substitution,
-- not retyped, because that is reviewed security code. The complete edit list:
--
--   1. mode CHECK                'synthetic'   ->  IN ('synthetic','live')
--   2. provider_instance pattern  ^synthetic:  ->  ^(synthetic|live):
--   3. create validation      rejects a mode outside {synthetic, live}
--   4. configure validation   same
--   5. claim guard            c.mode<>'synthetic' -> NOT IN ('synthetic','live')
--   6. commit/fail lease      same
--   7. reader freshness       c.mode='synthetic'  -> IN ('synthetic','live')
--   8. reader output          hardcoded 'mode','synthetic' now reports c.mode
--  10. reader actor lock      the re-check after a lock wait allowed only owner
--                            and org_admin, so a facility_admin admitted by edit
--                            9 would clear the role gate and then be thrown out
--                            by the lock. Both now agree.
--   9. reader authority       DECISION 2: facility_admin may read and sees only
--                            entities it holds a facility for; owner and
--                            org_admin keep the whole organisation. Same shape as
--                            insurance_policies_select, so there is one rule to
--                            remember instead of two.
--
-- Deliberately NOT changed:
--   * Creation still forces enabled = false. Turning a connection on stays a
--     separate, explicit configure call by a named actor.
--   * The TTL bound stays 1..86400. Decision 3 (24-hour freshness) is the
--     configured value 86400 held per connection, never a constant in here.
--   * Retention (decision 4) needs no DDL. The receiver already keeps a receipt
--     per release and discards only superseded bodies, which is soft-delete with
--     history retained. Proven by test rather than asserted in a comment.
--
-- Still inert when this finishes: no connection row, no credential, and the
-- client is gated separately by INSUREFLOW_LIVE_TRANSPORT_ENABLED.

BEGIN;

ALTER TABLE public.insureflow_receiver_connections
  DROP CONSTRAINT insureflow_receiver_connections_mode_check,
  ADD CONSTRAINT insureflow_receiver_connections_mode_check
    CHECK (mode IN ('synthetic', 'live'));

ALTER TABLE public.insureflow_receiver_connections
  DROP CONSTRAINT insureflow_receiver_connections_provider_instance_check,
  ADD CONSTRAINT insureflow_receiver_connections_provider_instance_check
    CHECK (provider_instance ~ '^(synthetic|live):[a-z0-9][a-z0-9_-]{0,63}$');
