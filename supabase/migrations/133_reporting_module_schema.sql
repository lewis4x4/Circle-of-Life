...
CREATE UNIQUE INDEX idx_report_templates_slug_org ON report_templates (coalesce(organization_id, '00000000-0000-0000-0000-000000000000'::uuid), slug)
WHERE
  deleted_at IS NULL;

CREATE INDEX idx_report_templates_lookup ON report_templates (status, category)
WHERE
  deleted_at IS NULL;
...
CREATE INDEX idx_report_template_versions_lookup ON report_template_versions (template_id, version_number DESC)
WHERE
  deleted_at IS NULL;
...
CREATE INDEX idx_report_saved_views_org ON report_saved_views (organization_id, owner_user_id)
WHERE
  deleted_at IS NULL;
...
CREATE INDEX idx_report_schedules_due ON report_schedules (organization_id, status, next_run_at)
WHERE
  deleted_at IS NULL;
...
CREATE INDEX idx_report_runs_org_created ON report_runs (organization_id, created_at DESC);
CREATE INDEX idx_report_runs_source ON report_runs (source_type, source_id, created_at DESC);
...
CREATE INDEX idx_report_exports_run ON report_exports (report_run_id, created_at DESC);
...
CREATE INDEX idx_report_packs_org ON report_packs (organization_id, active)
WHERE
  deleted_at IS NULL;
...
CREATE INDEX idx_report_pack_items_pack ON report_pack_items (pack_id, display_order)
WHERE
  deleted_at IS NULL;
...
CREATE INDEX idx_report_permissions_template ON report_permissions (template_id)
WHERE
  deleted_at IS NULL;
...
CREATE INDEX idx_report_benchmarks_metric ON report_benchmarks (organization_id, metric_key, benchmark_type)
WHERE
  deleted_at IS NULL;

CREATE TABLE report_nlq_mappings (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid (),
  organization_id uuid NOT NULL REFERENCES organizations (id),
  template_id uuid NOT NULL REFERENCES report_templates (id) ON DELETE CASCADE,
...
CREATE INDEX idx_report_nlq_mappings_template ON report_nlq_mappings (template_id)
WHERE
  deleted_at IS NULL;
