-- COL-756: preserve the one-click shift cycle while allowing explicit custom hours.
-- No schema/ACL changes: existing schedule RLS, version and overlap guards remain authoritative.
BEGIN;
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM jsonb_to_recordset(p_cells) AS x(staff_id uuid,shift_date date,shift_definition_id uuid,custom_start_time text,custom_end_time text) GROUP BY staff_id,shift_date HAVING count(*)>1) THEN RAISE EXCEPTION 'Duplicate employee/date cells'; END IF;
  ...
  FOR c IN SELECT * FROM jsonb_to_recordset(p_cells) AS x(staff_id uuid,shift_date date,shift_definition_id uuid,custom_start_time text,custom_end_time text) ORDER BY staff_id,shift_date LOOP
    ...
    IF c.custom_start_time IS NOT NULL OR c.custom_end_time IS NOT NULL THEN
  ...
  SELECT count(*) INTO expected_updates FROM public.shift_assignments a
    JOIN jsonb_to_recordset(p_cells) AS x(staff_id uuid,shift_date date,shift_definition_id uuid,custom_start_time text,custom_end_time text)
  ...
  UPDATE public.shift_assignments a
    SET deleted_at=CASE WHEN x.shift_definition_id IS NULL AND x.custom_start_time IS NULL THEN clock_timestamp() ELSE NULL END,
      shift_type=CASE WHEN x.custom_start_time IS NOT NULL THEN 'custom'::public.shift_type WHEN definition.id IS NULL THEN a.shift_type ELSE definition.roster_shift_type END,
      custom_start_time=CASE WHEN x.custom_start_time IS NOT NULL THEN x.custom_start_time::time WHEN definition.id IS NULL THEN a.custom_start_time ELSE definition.starts_at_local END,
      custom_end_time=CASE WHEN x.custom_start_time IS NOT NULL THEN x.custom_end_time::time WHEN definition.id IS NULL THEN a.custom_end_time ELSE definition.ends_at_local END,
      shift_definition_id=CASE WHEN x.custom_start_time IS NOT NULL THEN NULL WHEN definition.id IS NULL THEN a.shift_definition_id ELSE definition.id END,
      updated_by=auth.uid()
    FROM jsonb_to_recordset(p_cells) AS x(staff_id uuid,shift_date date,shift_definition_id uuid,custom_start_time text,custom_end_time text)
  ...
  FOR c IN SELECT * FROM jsonb_to_recordset(p_cells) AS x(staff_id uuid,shift_date date,shift_definition_id uuid,custom_start_time text,custom_end_time text) ORDER BY staff_id,shift_date LOOP
    IF (c.shift_definition_id IS NOT NULL OR c.custom_start_time IS NOT NULL) AND NOT EXISTS (
      ...
      IF c.custom_start_time IS NOT NULL THEN
        PERFORM public.edit_draft_schedule(w.id,'add',assignment_id,c.staff_id,c.shift_date,c.custom_start_time::time,c.custom_end_time::time);
      ELSE
        SELECT * INTO d FROM public.facility_shift_definitions WHERE id=c.shift_definition_id AND facility_id=w.facility_id AND organization_id=w.organization_id AND active AND deleted_at IS NULL;
        IF NOT FOUND OR d.starts_at_local=d.ends_at_local THEN RAISE EXCEPTION 'Active facility shift definition with distinct times required'; END IF;
...
BEGIN
  ...
  SELECT jsonb_agg(jsonb_build_object('staff_id',a.staff_id,'shift_date',a.shift_date+7,'shift_definition_id',d.id,
      'custom_start_time',CASE WHEN d.id IS NULL THEN a.custom_start_time END,
      'custom_end_time',CASE WHEN d.id IS NULL THEN a.custom_end_time END) ORDER BY a.staff_id,a.shift_date)
    INTO cells FROM public.shift_assignments a LEFT JOIN LATERAL (
