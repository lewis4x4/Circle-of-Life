...
BEGIN
  -- A direct UPDATE/DELETE already owns its assignment row when this trigger
  -- runs. Never wait for a parent held by a parent-first RPC: that RPC may be
  -- waiting for this same assignment. Abort this statement cleanly for retry.
  BEGIN
    SELECT * INTO w FROM public.schedules WHERE id=COALESCE(NEW.schedule_id,OLD.schedule_id) AND deleted_at IS NULL FOR UPDATE NOWAIT;
  EXCEPTION WHEN lock_not_available THEN
    RAISE EXCEPTION 'Schedule is being edited. Reload and retry.' USING ERRCODE='55P03';
  END;
