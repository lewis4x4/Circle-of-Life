BEGIN;
-- COL-326 withdrawal. Restores 374's corporate_deliverable_command exactly,
-- removing the fast revision pre-read added by 376 and amended by 377.
--
-- The pre-read was built on an unestablished root cause. The measured symptom
-- was HTTP 200 after 29.7s with nothing committed, not a slow 409; a fast path
-- only turns a slow 40001 into a prompt one and cannot produce that. Contention
-- lives on the task row lock taken by lock_operation_authority, and in the
-- canonical conflict the competing transaction has not committed, so the
-- revision still reads current and the pre-read never fires. It helped only a
-- stale view with unrelated concurrent activity.
--
-- Against that narrow benefit it twice broke idempotent replay: 376 refused a
-- byte-identical retry deterministically by running before the request-key
-- replay return, and 377 left the same harm as a race because its request-key
-- guard and its revision comparison read different READ COMMITTED snapshots.
-- Both reported a committed change as rejected, the exact harm request_key
-- exists to prevent. A compliance-critical SECURITY DEFINER function should not
-- carry that risk for an unproven benefit.
--
-- Body is byte-identical to 374's function; only CREATE becomes CREATE OR
-- REPLACE. The signature is unchanged, so 374's REVOKE/GRANT still apply.
-- The replay regression test added alongside 377 is retained: it passes here
-- because 374 has always replayed correctly.
