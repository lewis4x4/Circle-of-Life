...
--
-- Extends retrieve_evidence_hybrid_v2 with a `page_number` passthrough so the
-- Edge function can emit deep-link citation payloads of the form
--   /admin/knowledge/documents/<document_id>?anchor=<chunk_id>#page=<n>
-- without an extra round trip per source.
--
-- Postgres can't add a column to RETURNS TABLE without redefining the
-- function, so we DROP+CREATE — same signature otherwise (parameter list
-- unchanged), so callers don't need a code change at the signature level.
--
-- Also adds a thin read-only view, kb_chunk_anchor_v, that the new document
-- viewer route uses to render chunks in stable order with the anchor metadata
-- needed for in-page scroll.

DROP FUNCTION IF EXISTS public.retrieve_evidence_hybrid_v2 (
  text, text, text, integer, float, integer, uuid, text[], text[], uuid[], date
);
...
COMMENT ON FUNCTION public.retrieve_evidence_hybrid_v2 (text, text, text, integer, float, integer, uuid, text[], text[], uuid[], date) IS
  'KB-NEXT-06: RRF retrieval with audience/compliance/doc_id/effective_date filters plus page_number passthrough for citation anchors. service_role only.';
...
-- ---------------------------------------------------------------------------
-- Read-only view used by the new document viewer route. RLS on
-- public.chunks + public.documents already gates this; security_invoker keeps
-- the caller's tenancy context intact.
-- ---------------------------------------------------------------------------

CREATE OR REPLACE VIEW public.vw_kb_chunk_anchor
...
GRANT SELECT ON public.vw_kb_chunk_anchor TO authenticated;

COMMENT ON VIEW public.vw_kb_chunk_anchor IS
  'KB-NEXT-06: read-only projection of public.chunks + public.documents used by /admin/knowledge/documents/[id] to render anchor-scrollable chunk lists. RLS-safe (security_invoker).';
