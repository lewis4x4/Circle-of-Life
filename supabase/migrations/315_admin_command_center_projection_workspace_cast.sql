-- Live compatibility hotfix: the COL project's KB documents.workspace_id is
-- text, while its canonical values are UUID strings. Keep the shared KB schema
-- unchanged and make the Command Center projection compare like types.
...
BEGIN
  ...
  doctrine_docs AS MATERIALIZED (
    ...
    WHERE d.workspace_id = v_organization_id::text
