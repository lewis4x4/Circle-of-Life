...
SELECT o.id, NULL, 'risk.score_bands',
       ...
       DATE '2026-09-23',
       'Seeded by migration 491 (COL-710): the risk levels the nightly scorer used before they became configurable (critical below 50, high below 70, moderate below 85).'
...
SELECT o.id, NULL, 'survey_binder.due_window_days', '60'::jsonb,
       DATE '2026-09-23',
       'Seeded by migration 491 (COL-710): the 60-day survey binder window used before it became configurable.'
FROM public.organizations o
WHERE o.deleted_at IS NULL
ON CONFLICT DO NOTHING;
