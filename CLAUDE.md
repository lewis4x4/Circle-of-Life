...
| `npm run lint` | ESLint `src/` with `--max-warnings 0`, then `lint:constitution`. Pre-existing React Compiler-rule debt is baselined in `eslint-suppressions.json`; lint fails only on new violations. After fixing old ones run `npx eslint src --prune-suppressions` and commit the smaller file. Never re-run `--suppress-all`. |
