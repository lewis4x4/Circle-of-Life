...
SELECT pg_temp.batch_error($q$SELECT public.install_finance_credential_candidate(seed) FROM cf$q$,'55000','independent-vault-outage-not-invalid-response');
RESET ROLE; SELECT pg_temp.batch_assert(NOT EXISTS(SELECT 1 FROM public.finance_credential_receipts WHERE event='candidate_invalid'),'independent-vault-outage-no-false-invalid-receipt');
ROLLBACK;
