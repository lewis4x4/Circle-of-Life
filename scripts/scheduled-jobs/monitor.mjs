...
export function latestDue(schedule, now) {
  ...
  if (fields.length !== 5 || !fields.every(f => /^(\*|\*\/[1-9]\d*|\d+)$/.test(f))) {
    throw new Error('Unsupported cron schedule');
  }
  const limits = [[0,59],[0,23],[1,31],[1,12],[0,6]];
  fields.forEach((field,index) => {
    const value = Number(field.replace('*/',''));
    if (field !== '*' && (value > limits[index][1] || (!field.startsWith('*/') && value < limits[index][0]))) {
  ...
  for (let i=0;i<366*24*60;i++) {
    ...
    if (fields.every((field,index) => field==='*' || (field.startsWith('*/')
      ? (values[index]-limits[index][0])%Number(field.slice(2))===0 : values[index]===Number(field)))) return date;
  }
...
export function compareSecrets(jobs, vault, edge, projectRef) {
  return jobs.filter(j=>j.active).map(job=>{
