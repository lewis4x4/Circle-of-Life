...
test('dispatch claims durable attempt and records failure without recipients in its report',async()=>{
 const queries=[];
 const sql=async query=>{queries.push(query);if(query.startsWith('select id'))return[{id:delivery.id}];if(query.includes('system_alert_email_claim'))return[{delivery}];if(query.startsWith('select count'))return[{count:1}];return[];};
 const result=await dispatchEmail(sql,[{jobid:1,state:'error'}],[{jobid:1,jobname:"Quote's job",state:'error'}],'jobs',{});
 assert.equal(result.failed,1);assert.equal(result.provider_accepted,0);
 assert.ok(queries.some(q=>q.includes("'unconfigured'")));assert.ok(queries[0].includes("Quote''s job"));
 assert.ok(!JSON.stringify(result).includes('primary'));
});
