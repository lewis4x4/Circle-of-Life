...
const ANTI_PATTERNS = [
  ...
  // Removed during Phase D D1 sweep — `moonshot/*` directory deleted,
  // `AmbientMatrix` / `Sparkline` / `PulseDot` null stubs stripped from
  // JSX usage + imports, `V2Card` and typography moved to non-moonshot
  // paths, `KineticGrid` already lived at a non-moonshot path and
  // renders a flat semantic `<div className="grid">`. None of the
  // following regexes can fire on a clean tree anymore, so leaving them
  // in would either be permanent dead code (`moonshot/*`,
  // `AmbientMatrix`) or false flags on working semantic primitives
  // (`V2Card`, `KineticGrid`). `MOONSHOT comment` is also gone (codemod
  // 2 stripped them). Re-add if a regression re-introduces them.
];
