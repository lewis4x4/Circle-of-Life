...
The servicing extension exercises all five domain forms with outgoing commands parsed by the actual servicing schema before the API fixture responds. It reviews and approves a renewal package, simulates the former assignee leaving, records a new operational assignment without changing the prepared owner or payload, downloads the selected immutable version and generates a printable PDF. Incident selection uses only date, category and facility labels. Additional screenshots cover loss/exposure row editors and desktop/mobile/print servicing summaries. This remains component-level mocked-API evidence, not database lifecycle proof.
