...
const privateRoot = '/Users/brianlewis/.hermes/tmp/agent-runs/hfa-20260908-01a08335/full-supabase';
let config;
...
const sanitizedCommand = [process.execPath, 'scripts/finance-integration/authenticated-smoke.mjs', ...process.argv.slice(2).map(arg => arg.startsWith('--config=') ? '--config=<run-private-configuration>' : arg.startsWith('--suite=') ? `--suite=${suite}` : '<unsupported-argument>')];
...
async function verifyRuntime() {
  const identities = [];
  if (config.transport === 'docker-exec-gateway-proxy') {
    const proxy = JSON.parse(fs.readFileSync(path.join(privateRoot, 'gateway-proxy.private.json'), 'utf8'));
    const listeners = execFileSync('lsof', ['-nP', '-iTCP:59331', '-sTCP:LISTEN', '-Fp'], { encoding: 'utf8' });
    const command = execFileSync('ps', ['-p', String(proxy.pid), '-o', 'args='], { encoding: 'utf8' });
    const valid = proxy.project_id === config.projectId && proxy.port === 59331 && proxy.bind === '127.0.0.1' && proxy.container === 'supabase_kong_hfa-01a08335' && proxy.container_port === 8000 && listeners.split('\n').includes(`p${proxy.pid}`) && command.includes(`${privateRoot}/gateway-proxy.mjs`);
    assertions('run-owned native proxy listener and backend binding verified', valid, true, valid);
    identities.push({ service: 'native-gateway-proxy', pid: proxy.pid, port: proxy.port });
  }
  for (const [service, innerPort, hostPort] of [['db', '5432/tcp', 59322], ['kong', '8000/tcp', 59321], ['auth'], ['rest']]) {
    const container = await new Promise((resolve, reject) => {
      const req = http.get({ socketPath: '/Users/brianlewis/.docker/run/docker.sock', path: `/v1.54/containers/supabase_${service}_hfa-01a08335/json` }, response => {
        const chunks = [];
        response.on('data', chunk => chunks.push(chunk));
        response.on('end', () => {
          try {
            if (response.statusCode !== 200) throw new Error('Run-owned local service unavailable');
            resolve(JSON.parse(Buffer.concat(chunks).toString('utf8')));
          } catch { reject(new Error('Run-owned local service unavailable')); }
        });
      });
      req.setTimeout(30000, () => req.destroy(new Error('Local runtime identity check timed out')));
      req.on('error', () => reject(new Error('Local runtime identity check failed')));
    });
    const matches = container.State?.Running === true && container.Config?.Labels?.['com.supabase.cli.project'] === config.projectId && (!innerPort || container.NetworkSettings?.Ports?.[innerPort]?.some(port => port.HostPort === String(hostPort)));
    assertions(`${service}: current container identity and port binding verified`, matches, config.projectId, matches ? config.projectId : 'mismatch');
    identities.push({ service, container_id: container.Id });
  }
  return identities;
}
function sql(statement) {
  ...
  const result = spawnSync('/Applications/Docker.app/Contents/Resources/bin/docker', ['--host', 'unix:///Users/brianlewis/.docker/run/docker.sock', 'exec', '--user', 'postgres', 'supabase_db_hfa-01a08335', 'psql', '-X', '-A', '-t', '-U', 'postgres', '-d', 'postgres', '-v', 'ON_ERROR_STOP=1', '-c', statement], {
...
try {
  ...
  const validApi = config.apiUrl === 'http://127.0.0.1:59321' || (config.apiUrl === 'http://127.0.0.1:59331' && config.transport === 'docker-exec-gateway-proxy');
  if (config.projectId !== 'hfa-01a08335' || !validApi || config.database?.host !== '127.0.0.1' || config.database?.port !== 59322 || config.database?.name !== 'postgres' || config.database?.user !== 'postgres' || typeof config.anonKey !== 'string' || typeof config.serviceRoleKey !== 'string' || typeof config.database?.password !== 'string' || !config.anonKey || !config.serviceRoleKey || !config.database.password) throw new Error('Verified disposable local target required');
  configured = true;
...
const filename = `${started.toISOString().replaceAll(':', '-')}-authenticated-${suite}.json`;
const report = { schema_version: 1, run_id: runId, suite, status, evidence_layer: 'full-local-GoTrue-PostgREST', target_identity: configured ? { project_id: config.projectId, api_url: config.apiUrl, database_port: config.database.port, transport: config.transport ?? 'direct-published-port', containers: runtimeIdentities } : null, command: sanitizedCommand, exit_code: status === 'PASS' ? 0 : status === 'BLOCKED_EXTERNAL' ? 2 : 1, commit: checkoutCommit, source_sha256: sourceSha256, schema_bootstrap_sha256: schemaBootstrapSha256, fixture_seed_sha256: checksum(JSON.stringify(fixture)), cases, counts: { total: cases.length, passed: cases.filter(item => item.status === 'PASS').length, failed: cases.filter(item => item.status === 'FAIL').length, blocked: cases.filter(item => item.status === 'BLOCKED_EXTERNAL').length, skipped: 0 }, failure, started_at: started.toISOString(), completed_at: new Date().toISOString(), duration_ms: Date.now() - started.getTime(), limits: ['Synthetic isolated local Auth/PostgREST only. Not hosted production, provider or business acceptance.'] };
