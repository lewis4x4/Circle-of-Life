"""Generate migration 462 (role consolidation) from a full local replay.
...
Usage: point /tmp/loc-psql.sh at a replay that has every migration through 461
applied, then: python3 scripts/role-sweep/generate-nurse-to-med-tech.py > supabase/migrations/462_...sql
"""
import re
import subprocess
import sys
...
MARKETING_TABLES = re.compile(r"^(referral_|reputation_)")
MARKETING_FUNCTIONS = re.compile(r"^(referral_|reputation_)")
...
def rewrite_list(body, negative, add_marketing):
    ...
    if not any(n in TARGET for n in names) and not (add_marketing and "coordinator" in names):
        return None
    ...
    if negative:
        ...
        for n in names:
            ...
            if t not in out:
                out.append(t)
        if add_marketing and "coordinator" in names and "marketing" not in out:
            out.append("marketing")
    return sep.join(f"'{n}'{cast}" for n in out)


def swap(text, where, add_marketing=False):
    def in_sub(m):
...
-- supabase/migrations through 461, checked against production before generation.
...
-- or reputation surface grants the coordinator, plus lead writing.
...
pol = q(f"""select schemaname, tablename, policyname, coalesce(qual,''), coalesce(with_check,'')
 from pg_policies where (coalesce(qual,'')||coalesce(with_check,'')) ~ '''({ROLE_RE})'''
   or (tablename ~ '^(referral_|reputation_)' and (coalesce(qual,'')||coalesce(with_check,'')) ~ '''coordinator''')
 order by 1,2,3""")
problems = []
print(f"-- {len(pol)} policies")
for s, t, p, qual, chk in pol:
    mk = bool(MARKETING_TABLES.match(t))
...
fns = q(f"""select p.oid, n.nspname, p.proname from pg_proc p join pg_namespace n on n.oid=p.pronamespace
 where n.nspname in ('public','haven') and (p.prosrc ~ '''({ROLE_RE})'''
   or (p.proname ~ '^(referral_|reputation_)' and p.prosrc ~ '''coordinator'''))
 order by 2, 3, 1""")
hand = {}
print(f"\n-- {len(fns)} functions")
for oid, nsp, name in fns:
    ...
    if name in HAND_WRITTEN:
        hand[name] = d
        continue
    new = swap(d, name, bool(MARKETING_FUNCTIONS.match(name)))
