 ...
 * run the assignment acceptance assertions against it and print the result
 * table.
 *
 * Proves the three cases of the generator's assignment fallback chain: a
 * facility with a resident split, a facility where staff are scheduled and
 * nobody split the residents, and a facility with nobody scheduled at all.
 *
...
const acceptance = path.join(root, "scripts", "smart-rounding", "assignment-acceptance.sql");
...
const database = `haven_assignment_${process.pid}_${Date.now()}`;
...
try {
  ...
  console.log(`[assignment] replayed ${files.length} migration files`);
  ...
  console.log("[assignment] PASS");
} catch (cause) {
  failed = cause;
} finally {
  if (!keep) run("dropdb", [...connection, database]);
  else console.log(`[assignment] kept database ${database}`);
}
