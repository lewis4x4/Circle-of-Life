 ...
 * run the escalation ladder acceptance assertions against it and print the
 * result table.
 *
...
const acceptance = path.join(root, "scripts", "smart-rounding", "escalation-ladder-acceptance.sql");
...
const database = `haven_escalation_ladder_${process.pid}_${Date.now()}`;
...
try {
  ...
  console.log(`[escalation-ladder] replayed ${files.length} migration files`);
  ...
  console.log("[escalation-ladder] PASS");
} catch (cause) {
  failed = cause;
} finally {
  if (!keep) run("dropdb", [...connection, database]);
  else console.log(`[escalation-ladder] kept database ${database}`);
}
