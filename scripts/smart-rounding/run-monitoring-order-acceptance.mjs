 ...
 * run the Monitoring Orders acceptance assertions against it and print the
 * result table.
 *
...
const acceptance = path.join(root, "scripts", "smart-rounding", "monitoring-order-acceptance.sql");
...
const database = `haven_monitoring_orders_${process.pid}_${Date.now()}`;
...
try {
  ...
  console.log(`[monitoring-orders] replayed ${files.length} migration files`);
  ...
  console.log("[monitoring-orders] PASS");
} catch (cause) {
  failed = cause;
} finally {
  if (!keep) run("dropdb", [...connection, database]);
  else console.log(`[monitoring-orders] kept database ${database}`);
}
