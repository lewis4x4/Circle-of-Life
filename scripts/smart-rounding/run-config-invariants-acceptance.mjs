 ...
 * run the cadence and escalation configuration invariant assertions against it
...
const acceptance = path.join(root, "scripts", "smart-rounding", "config-invariants-acceptance.sql");
...
const database = `haven_config_invariants_${process.pid}_${Date.now()}`;
...
try {
  ...
  console.log(`[config-invariants] replayed ${files.length} migration files`);
  ...
  else console.log(`[config-invariants] kept database ${database}`);
}
