 ...
 * run the compliance honesty acceptance assertions against it and print the
 * result table.
 *
...
const acceptance = path.join(root, "scripts", "smart-rounding", "compliance-honesty-acceptance.sql");
...
const database = `haven_compliance_honesty_${process.pid}_${Date.now()}`;
...
try {
  ...
  console.log(`[compliance-honesty] replayed ${files.length} migration files`);
  ...
  console.log("[compliance-honesty] PASS");
} catch (cause) {
  failed = cause;
} finally {
  if (!keep) run("dropdb", [...connection, database]);
  else console.log(`[compliance-honesty] kept database ${database}`);
}
