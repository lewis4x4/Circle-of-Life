 ...
 * run the Monitoring Order boundary acceptance assertions against it and print the result
 * table.
 *
 * Proves that Monitoring Order suppression is per window rather than per
 * resident, that entering an order excuses only the windows it covers, and that
 * ending an order hands the resident back to the standard cadence immediately
 * rather than at the next generator tick.
 *
...
const acceptance = path.join(root, "scripts", "smart-rounding", "order-boundary-acceptance.sql");
...
const database = `haven_order_boundary_${process.pid}_${Date.now()}`;
...
try {
  ...
  console.log(`[order-boundary] replayed ${files.length} migration files`);
  ...
  console.log("[order-boundary] PASS");
} catch (cause) {
  failed = cause;
} finally {
  if (!keep) run("dropdb", [...connection, database]);
  else console.log(`[order-boundary] kept database ${database}`);
}
