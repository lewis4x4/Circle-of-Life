...
const seedData = await import("../src/seedData.js");
const { demoSeedState } = await import("./demoFixture.mjs");
...
check("b0) real reset starts from an empty onboarding shell", gate0.pass === false && (state.mvpData?.M5?.residents || []).length === 0 && (state.documents || []).length === 0 && state.modules.every((module) => module.status === "not_started"), gate2.blockers.join(" | "));

storage.set(seedData.STORAGE_KEY, JSON.stringify(demoSeedState));
const migratedState = stateApi.loadState();
check("b0.1) legacy demo localStorage is discarded", (migratedState.documents || []).length === 0 && !(migratedState.decisionLog || []).some((entry) => entry.id === "dec-seed-1") && migratedState.modules.every((module) => module.status === "not_started"));

let realDraftState = stateApi.resetState();
realDraftState = stateApi.appendDecisionLog(realDraftState, {
  ...
  relatedId: realDraftState.facility?.id || "fac-homewood"
});
realDraftState = stateApi.updateProgramField(realDraftState, "sponsor", "Real Round 1 Sponsor Draft");
const reloadedRealDraft = stateApi.loadState();
check("b0.2) real post-reset draft state persists across reload", reloadedRealDraft.program.sponsor === "Real Round 1 Sponsor Draft" && (reloadedRealDraft.documents || []).length === 0);

const round1ImportState = JSON.parse(readFileSync(new URL("../data/homewood-round1-state.json", import.meta.url), "utf8"));
stateApi.saveState(round1ImportState);
stateApi.appendDecisionLog(round1ImportState, {
  actor: "user",
...
const reloadedRound1State = stateApi.loadState();
check("b0.3) Round 1 import persists across reload",
  (reloadedRound1State.mvpData?.M3?.rooms || []).length === 20
    && (reloadedRound1State.documents || []).length === 4
    && (reloadedRound1State.ingestionGaps || []).length === 11
    && (reloadedRound1State.ingestionReviewQueue || []).length === 10
    && (reloadedRound1State.decisionLog || []).some((entry) => entry.actionType === "round1_real_import_loaded")
    && !(reloadedRound1State.decisionLog || []).some((entry) => entry.id === "dec-seed-1")
);

state = stateApi.saveState(JSON.parse(JSON.stringify(demoSeedState)));
gate0 = gates.evaluateGate0(state);
gate2 = gates.evaluateGate2(state);
check("b) explicit demo fixture Gate 0 is ready", gate0.pass === true, `${gate0.criteria.filter((c) => c.pass).length}/${gate0.criteria.length}`);
check("c) explicit demo fixture Gate 2 starts blocked", gate2.pass === false, gate2.blockers.join(" | "));
...
check(
  ...
  Boolean(m17Metrics.ownerName && m17Metrics.scopeStatus && m17Metrics.status && typeof m17Metrics.score === "number" && m17Metrics.evidenceCount >= 5 && m17Metrics.staleCount === 0 && m17Metrics.exceptionCount >= 0 && m17Metrics.nextAction)
);
