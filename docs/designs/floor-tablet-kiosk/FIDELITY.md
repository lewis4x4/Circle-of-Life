...
| 03-floor-now | MATCH | §2.1 to §2.4. Spec: the Tasks rows are the person's open witness statements (spec 40 §6.3 "the signed-in person's assigned tasks"; Haven has no staff task list with due times, so "Water pitchers" and "Shower assist" cannot exist). Data: check sub-lines read "Safety check" (no other check kind in the data), rail tile order and notes come from real escalations, watches and holds, "All 35 residents" counts George Adams on hospital hold, the round chip reads "8:05 Rounds: 11 of 12 charted" (Ruth's 8:00 check is charted but unassigned, so it is not counted as hers). |
