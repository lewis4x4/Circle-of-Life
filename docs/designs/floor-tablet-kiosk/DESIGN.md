...
| `05-floor-check` | `/floor/check/[taskId]`, overdue check | Title and room, "10 min over" pill, How is she (8 chips, one pick), Where is she (location chips from `observation_vocab`), What is she doing (state chips), Meals, Mood and Medications (vocabulary chips, at least one), Did you help with (3, any), Anything wrong (5, any), Why late (required when over), footer with charted-by line, Cancel and Save check |
