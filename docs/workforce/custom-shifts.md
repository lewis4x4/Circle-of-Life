...
Migration503 extends the existing atomic bulk-save and copy RPCs. Explicit custom times are distinct from Off; preset plus custom overrides are rejected. Existing assignment IDs, metadata, scope/RLS, version and overlap guards remain intact. No payroll rules are changed.
