...
Source revision locks and policy revisions protect approval against concurrent changes. Unrelated source changes can refresh revision metadata only when the complete business snapshot is unchanged. Source pagination is complete and revision checked. Approved PDF bytes are SHA-256 checked before delivery; HTML/CSV are stored once with the same approval. CSV formula prefixes and HTML text are escaped. Print documents disclose the PDF's ASCII fallback; HTML/CSV retain exact Unicode text.
