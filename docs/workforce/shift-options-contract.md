...
- A concurrent older split-shift release introduced migration 514. Its historical file is retained; forward migration 518 restores the shared configurable grid RPCs after it. Fixed universal color/hour helpers are superseded by the preset model. The original bundle budget is retained.
