...
The subsequent native replay **passed 505 migration files, 122 SQL probes, seven acceptance suites and 105 care-event parity cases** on current main, including the already-hosted family migration 499, onboarding migration 500, and Workforce policy migration 501. The SQL policy scanner failed before migration 501 and passed after it; a behavior probe failed against the old hardcoded cutoff and passed with the configurable setting. Local bundle checks passed at 448.9 kB for `/admin` and 449.4 kB for `/admin/executive` under the unchanged 450 kB cap. Staging has 496–498 and 501–502; its enabled facility and four devices remain intact. Production has 494/495, independent 499/500, and Workforce 496–498 and 501. Production 502 awaits refreshed CI. Current-base required PR CI, merge and post-merge CI, production migration/application/function deployment, and hosted readback remain **not yet completed**.
...
The branch incorporates main through migration 495 and now has a verified source sequence through 502. It needs current-base required CI before merge. The pinned import receipt covers 22 files; its sanitized copy is `evidence/prerequisite-provenance.json`.
...
| 494–495 | Current main source from released PR #814; the floor application merged independently and the shared clock helper is authoritative |
...
| 501 | Configurable rounding owner clock evidence age, default 960 minutes; retains the already-applied source record |
| 502 | Forward reconciliation uses the floor clock helper from current main and applies the configurable age cutoff to its result |
...
Brian will provide ADP/pay-rule details later. Payroll approval/snapshots, immutable period/export packets, ADP mapping/layout and submission remain pending. Open shifts, staff time-off/pick-up requests, staff notifications, configured staffing/compliance rules and credential evidence consolidation remain subsequent work. The floor application merged independently through PR #814; Workforce activation, device enrollment and staff acceptance remain separate operating decisions. No vendor retirement, roster sign-off, human acceptance or production release of this draft is claimed.
