...
- Residents are visible to any role that is not `family` holding the facility grant (`013_resident_profile_rls.sql`). `family` sees only linked residents through `haven.can_access_resident(id)`. Admitting and discharging are `owner`, `org_admin`, `facility_admin`, `med_tech` (migration 468 folded the retired `nurse` and `caregiver` grants into `med_tech`).
