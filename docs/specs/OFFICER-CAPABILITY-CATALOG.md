...
| `officer.facility_coverage` | Operator-maintained honesty table: `live`, `demo` or `none` per facility. Seeded Homewood = live, the other four = demo. Every read labels each `by_facility` row with its coverage and appends the fixed qualifier "Figures for facilities marked demo come from seeded demonstration data, not operations." whenever a demo facility is covered. A facility with **no row here** reports `coverage: "unknown"` and raises its own qualifier; the label is never missing. `none` means "this facility has no records" and its (zero) numbers are still summed, which is why the label exists. |
...
Check order inside `officer_execute`: key enabled and allowed for the capability; per-key advisory lock (`pg_advisory_xact_lock(hashtextextended(...))`, the 336 pattern); rate limits (four windows, below); clock skew 60 s; nonce insert (duplicate is `23505 replayed_request`); officer registered, email matches, role matches (`principal_unknown`), active and inside the validity window (`principal_inactive`); capability enabled (`capability_denied`) and version current (`version_conflict`); role allowed (`capability_denied`); assurance (`assurance_required`); args exact key set (`{}` for every v1 read; `invalid_args` otherwise); `catalog_hash` well formed; for commands, `intent.intent_id` present and idempotency (same intent + same request hash replays the stored result with `replayed: true`; different hash is `22023 idempotency_key_reused`). Then the audit row, then `CASE p_capability WHEN ... END`.

**Rate limits.** Independent success and refusal windows at two levels over `officer.audit_events`, all answering `429 rate_limited`. Recorded refusals never consume a success budget, so a caller holding the secret cannot starve the officers by generating refusals, and a burst of refusals aimed at one seat cannot lock it out of real work:

| Level | Successes (`outcome in ('ok','replayed')`) | Refusals (`outcome = 'refused'`) | Why |
|---|---|---|---|
| per key, last minute | 60 | 120 | bounds legitimate load; bounds free guesses at officer refs and capability names |
| per officer ref, last minute, across keys | 30 | 60 | one compromised or looping seat cannot starve the other four, and refusals aimed at a seat cannot lock it out |

`rate_limited` is recorded like any other refusal, deduplicated to one row per key per UTC minute by a partial unique index on `(key_id, date_trunc('minute', occurred_at at time zone 'UTC')) where outcome = 'refused' and error_code = 'rate_limited'` (`on conflict do nothing` inside `officer_record_refusal`). A sustained flood therefore stays attributable, growth is bounded at 1440 rows per key per day, and the rows land in the refusal windows only, so they cannot lock a flooded key out of successful work.

`officer_record_refusal` is the only writer of refusal rows. It writes nothing unless the key id exists in `officer.gateway_keys` (enabled or not), the code is in the published set, and the Edge Function has verified the signature. The key-existence check lives inside the function, not only in the Edge Function, and the replay probe proves it. The two read-only doors are structurally write-proof: `officer_key_secret_env` is `language sql stable` and `officer_catalog` is `plpgsql stable`, and PostgreSQL refuses an INSERT inside a non-volatile function.
...
**Grant durability.** Migration `308_revoke_anon_security_definer_rpc_execute.sql` is an idempotent sweep that grants every public SECURITY DEFINER function it does not recognise to `authenticated`. All four doors are now named in 308's own service-role-only list, so a re-run of 308 after 339 leaves them service-role-only; this was verified empirically, and before the amendment the same re-run flipped all four to `authenticated`. Containment used to rest on 308 sorting before 339 in an ordered replay, which is ordering luck rather than design. `supabase/tests/review_officer_catalog.sql` is the tripwire and runs in every replay. `CREATE OR REPLACE FUNCTION` resets EXECUTE to PUBLIC, so the REVOKE/GRANT block at the end of 339 must run again after any future redefinition.

## 3. The catalog (`catalog_version` `2026-09-11.2`)

All reads: `kind = read`, `version 1`, `params = []`, `allowed_officer_roles = owner, ceo, cfo, coo, ctdo`, `assurance = session`, `phi_class = none`. Every read is organization-wide for the officer's registered organization (COL, `00000000-0000-0000-0000-000000000001`), and returns `data.facilities_covered` and `data.by_facility` (`[{facility, coverage, value, ...}]`, facility name as the only label, at most 50 rows). `as_of = generated_at = now()`, `freshness.state = current`.

**One facility set.** The headline `value` and `data.by_facility` both derive from `officer.scope_facilities`, which is the organization's facilities with `deleted_at is null`, ordered by name, capped at 50. This is structural, not a convention: a headline scoped to the whole organization while the breakdown under it showed only some facilities is how a total comes to disagree with its own breakdown, and the catalog exists to stop exactly that. **The headline always equals the sum of the breakdown**, and `supabase/tests/review_officer_catalog.sql` asserts that identity for all six reads against a fixture containing a soft-deleted facility.

**Soft-deleted facilities are out of scope, on both sides.** A facility the organization has soft-deleted contributes no residents, invoices, incidents or certifications to the headline and appears in no `by_facility` row. This is the deliberate ruling: a deleted facility is not part of current operations, and Haven's own executive KPI dashboard is the only figure that counts it, which is why the AR qualifier already warns the two will not agree. Rows dropped this way are never dropped silently: they are counted into `missing.count` and named in a qualifier that gives the count, the reason, and for the two money reads the excluded cents. The 50-facility cap behaves identically.

**Unclassified coverage reads `unknown`, never absent.** A facility with no `officer.facility_coverage` row reports `coverage: "unknown"` and raises the qualifier "At least one facility in this figure has coverage unknown…". Its numbers are included. The key is never stripped, because an absent coverage key and an unknown coverage look identical on screen and only one of them is true. This is the default state of every facility onboarded from now on, so it is the common case, not the edge case.
...
Rate limits are enforced in the database (the four windows above), not with the in-memory `_shared/rate-limit.ts`, so they survive cold starts and count across instances.

## 5. Tests

- `supabase/tests/review_officer_catalog.sql` runs after every migration in both replay paths (`npm run migrations:verify:pg`: Docker in CI, native locally) and rolls back. It asserts: seven tables with RLS and no client privilege; `anon`/`authenticated` cannot execute either door or the two helpers and `service_role` cannot read the tables directly (the 308 tripwire); `haven.stand_up_command` grants unchanged; registry empty, key disabled and coverage seeded at ship; catalog shape, the AR meaning naming its predicate, allowlist filtering; every refusal code in contract order (`key_disabled`, `principal_unknown`, `principal_inactive`, `capability_denied`, `version_conflict`, `invalid_args`, `expired_request`, `assurance_required`, `replayed_request`, `rate_limited`); refusals leave no audit row and no nonce; `occupied_beds` (2+1 fixture with a discharged, a deleted and a cross-organization resident excluded), exact envelope key set, no forbidden key, no person string, coverage labels and the demo qualifier present only when a demo facility is covered; `licensed_capacity` with `occupancy_percent`; `open_ar_balance` `no_data` without invoices, then 11000 cents from a sent, an overdue and a partial invoice while **a draft carrying a 2345 balance**, a voided, a paid and a written-off row are excluded, buckets 300 / 700 / 10000 / 0 / 0 and oldest 45 days, then a valid 0 once everything is paid; `billed_revenue_mtd` `no_data` without invoices then 20000 cents from the same rows with draft, voided and written-off excluded and the period bounds; incidents and certifications inside and outside their windows; ping writes one receipt and one audit row, replays with `replayed: true`, refuses a different request hash under the same intent; audit and receipts immutable; refusal recording accepts only published codes and only registered keys; the rate-limit windows, including that 60 recorded refusals do not block the next legitimate call, 120 refusals close only that key, 60 successes close that key, 59 refusals aimed at one officer do not block that officer's real work while 60 do, 30 successes close that officer while another seat keeps working, and 25 `rate_limited` recordings in one minute leave exactly one row per key while ordinary refusals are all kept; zero licensed beds report an unknown percent; nonce pruning.
- The reconciliation and coverage regressions, on their own gateway key and their own seat so they do not spend the rate-limit budgets the windows above depend on: a fixture adds a **soft-deleted** facility and an **unclassified** facility, each carrying a resident, an open invoice billed this month, an incident and a certification. For all six reads it asserts that the deleted facility never reaches the breakdown, that the unclassified one always does, that **the headline equals the sum of `by_facility`**, that `missing.count` is 1 rather than the rows being dropped silently, that a qualifier names what was excluded, that every `by_facility` row still carries a `coverage` key and the unclassified one reads `unknown`, and that the scope and unknown-coverage qualifiers are present. Then named figures: 4 occupied beds over 35 covered licensed beds for 11.4 percent, 111 cents open with 777 cents named as excluded, the unknown qualifier disappearing once the facility is classified while its `none` numbers still count, and undeleting the facility restoring headline and breakdown together. Both halves were confirmed to fail against the pre-fix code.
...
**Required, in the same session, immediately before the migration file:**

```sql
SET lock_timeout = '5s';
```

This is a step, not a suggestion. 339 creates foreign keys from `officer.facility_coverage` and `officer.federated_officers` into `public.facilities`, `public.organizations` and `public.user_profiles`, which takes `ShareRowExclusiveLock` on all three for the length of the transaction. That lock conflicts with `RowExclusiveLock`, so INSERT, UPDATE and DELETE on those three tables block while it is held. Reads are unaffected: staff looking at charts, residents and schedules see nothing. The migration itself takes about 0.27 s measured against a full 001..338 replay, with no table rewrite, no index build over existing rows and no validation scan.

The risk is not the duration, it is the queue. Nothing else in the apply path sets a timeout, so if a long transaction is already holding a conflicting lock on any of those three tables, 339 waits indefinitely **and every later write to them queues behind its pending lock request**. That is a facility-wide write outage caused by a 0.27 s migration. With the timeout set, the migration aborts after five seconds having applied nothing, and can simply be retried. With it set, 339 is safe to apply mid-shift.

Then apply the file through the normal linked-CLI release path used for every Haven release, after `supabase migration list --linked` confirms `338` is applied. Then `npm run migrations:verify:remote` probes the `officer_catalog` door (row `migration: "339"` in `scripts/verify-remote-schema.mjs`; an unknown key raising `key_disabled` proves the door exists and refuses).
...
- `public.officer_key_secret_env` and `public.officer_record_refusal` are two extra service-role-only doors (the second is now an accepted pattern in the contract). The first exists because the Edge Function holds the secret by name only and the name lives in the key row. The second exists because a refusal raised inside `officer_execute` rolls its own audit row back. `rate_limited` is recorded, deduplicated to one row per key per UTC minute, and counts only toward the refusal windows, so a flood stays attributable without locking the key out through its own refusals.
