...
8. **Who can run a check.** Board Check: the roles that can admit and discharge, which are `owner`, `org_admin`, `facility_admin` and `med_tech`. Staff Check: the roles that can deactivate staff, which are `owner`, `org_admin` and `facility_admin`. Both on granted facilities only.
