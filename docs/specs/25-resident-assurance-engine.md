...
## Section 2 completion integrity contract (migration 331)

A submission carries a stable UUID `requestId` and original `observedAt` from before the first network attempt. Both remain unchanged for retries and offline delivery. Legacy outbox entries use their existing queue ID. Completion commits the existing locked task/log/exception core, integrity flags and an immutable request receipt in one database transaction. The server derives first-entry time, mode and completion status. Equivalent authorized retries return that saved receipt; changed clinical payload, actor, task or a competing completion conflicts. Transport mode and server-derived timestamps do not change receipt identity. A fresh current-actor/session/version/staff/facility/assignment check precedes replay.

The former public core is preserved as a private `haven.complete_rounding_task_core`; API roles cannot execute it. The public service-only wrapper serializes request keys and staff/facility integrity counting. Late-entry and suspicious-pattern detection retain their existing thresholds and create evidence transactionally. A failure at any write, including receipt or audit, rolls the whole completion back.

Ordinary API clients cannot insert or edit observation logs, insert/delete tasks, or change protected task identity/completion fields. Managers retain the existing audited excusal path with current authority, a nonblank reason and their own actor identity. A completed task cannot be overwritten by a delayed excusal or service escalation. Accepted logs and receipts are immutable; any future correction workflow must append attributable evidence instead of rewriting an accepted observation. No historical record is deleted by this migration.

This contract covers observation completion and protected task/log writes. Watch lifecycle, plan-save transactions and delivery of escalations remain governed by their existing contracts and later roadmap scopes. Apply the database and paired application only after independent review; new online clients require migration 331. Pre-upgrade queued entries retain their author and original observed time. Pre-migration completions have no retroactively fabricated request receipts and conflicting legacy retries remain in the Outbox for reconciliation.
