...
| Certification Dashboard | `/admin/certifications` | Grid: staff names × certification types. Green (current), Yellow (expiring in 90 days), Red (expired or missing). Drill down to renew. **Certifications CSV download** (Track D30): full-row export for **`staff_certifications`** visible in the hub (same facility scope as the list; list loads up to **500** rows); **hub filter scope** (Track D81): export matches **search**, **timeline**, and **record status** filters when any are set (`_filtered` filename suffix); includes **`staff_display_name`**. |
...
| Shift swap queue | `/admin/shift-swaps` | List of **`shift_swap_requests`** for scheduling oversight (requesting → covering staff labels). **CSV** (Track D36): up to 500 rows with display names. **Approve / deny** for **pending** rows (Track D37): sets `approved_*` or `denied_reason`; facility-scoped when a facility is selected. |
...
**D30:** **`/admin/certifications`** — **Download certifications CSV** queries **`staff_certifications`** rows (`deleted_at` null) with joined **`staff_display_name`**, **RFC-style** CSV. Scope matches the matrix list (facility filter when valid). **No** new DDL.

**D81:** Same hub — **Download certifications CSV** exports only rows that match the current **hub filters** (search, timeline, record status); **`staff-certifications-YYYY-MM-DD_filtered.csv`** when any filter is non-default; **`staff-certifications-YYYY-MM-DD.csv`** when all filters are default. **No** new DDL.
...
**D79:** Same hub — **Download time records CSV** applies the **approval** filter (**All** / **Approved** / **Not approved**) from **`AdminFilterBar`** to the export query; filename **`time-records-YYYY-MM-DD_approved.csv`** or **`_not_approved.csv`** when filtered; **search** remains **list-only** (subtitle on the punches card).

**D60:** **`/admin/time-records`** — **Approve all pending** sets **`approved`**, **`approved_at`**, **`approved_by`**, **`updated_by`** for all **loaded** rows with **`clock_out`** set and **`approved`** false, scoped to the **selected facility**. Requires **RLS** roles that may update facility time records (**`owner`**, **`org_admin`**, **`facility_admin`** per existing **`time_records`** UPDATE policy). **No** new DDL.
...
**D33:** **`/admin/schedules`** — **Download schedule weeks CSV** queries **`schedules`** rows (`deleted_at` null), **RFC-style** CSV. Scope matches the hub list (facility filter when valid). **No** new DDL.

**D82:** Same hub — **Download schedule weeks CSV** exports only rows that match the current **hub filters** (search, week status); **`schedule-weeks-YYYY-MM-DD_filtered.csv`** when any filter is non-default; **`schedule-weeks-YYYY-MM-DD.csv`** when all filters are default. **No** new DDL.
