...
| PH1-P02 | Migrations applied / list aligned remote | **VERIFY** | owner | 2026-04-09 | Repo includes **001–115**; owner keeps `supabase migration list` Local/Remote aligned on target (includes **`110`–`111`** for JWT `app_role` + `user_profiles.updated_by`, **`112`–`113`** transport/mileage, **`114`** org transport settings, **`115`** competency certificate storage as applicable) |
