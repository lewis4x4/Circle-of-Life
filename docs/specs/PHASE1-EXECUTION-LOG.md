...
| PH1-A04 | Wrong role cannot open other shell routes | PARTIAL | agent | 2026-08-19 | Production smoke 2026-08-19T20:00:08Z: owner `/caregiver` → `/admin/executive`; caregiver `/admin/residents` → `/caregiver`. facility_admin + family logins timed out waiting for post-login navigation. `verdict.pass=false`; `all_cross_shell_ok=true` for the two sessions that authenticated. |
...
| PH1-OA04 | **PH1-P06** — Pro plan, **BAA** before PHI, **PITR** enabled | PARTIAL | Brian Lewis / agent | 2026-08-19 | Supabase Pro plan and BAA owner-confirmed (2026-05-11). Re-probe 2026-08-19: `supabase backups list --project-ref manfqmasfqppukpobpld -o json` — completed physical backups through 2026-08-19T12:37Z, `walg_enabled: true`, **`pitr_enabled: false`**. CLI has `backups list` / `backups restore` only — owner must enable PITR in Dashboard → Database → Backups. See [COL-GO-LIVE-READINESS-CHECKLIST.md](./COL-GO-LIVE-READINESS-CHECKLIST.md). |
