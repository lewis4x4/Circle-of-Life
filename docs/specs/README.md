...
**Repo migrations:** **`001`–`118`** — verify with `npm run migrations:check` and `npm run migrations:verify:pg` before release.

**Remote migration parity:** Re-run `supabase migration list` after **`118`** ships to remote; prior note was **001–117** on **`manfqmasfqppukpobpld`**. Authoritative record: [PHASE1-ENV-CONFIRMATION.md](./PHASE1-ENV-CONFIRMATION.md).
...
| Phases 3–6 — live proof / operational hardening | **Incomplete** until Track A closes; **Tracks B–C engineering complete** (see sections below); **Track D** Core **D1–D10** + Enhanced **D12–D43** shipped (see [TRACK-D-PHASE6-PASS.md](./TRACK-D-PHASE6-PASS.md)) — further Enhanced backlog per plan | Same tables + [TRACK-D-PHASE6-PASS.md](./TRACK-D-PHASE6-PASS.md) |
...
**Next free migration number:** **`119`** — use for all new DDL after updating this README and the relevant spec.
...
4. **Track D** — **Segments D1–D10** (2026-04-09) + Enhanced **D12–D43** (incl. Module 11 hubs **D29–D33** + **D35–D37**, **D34** shared CSV helpers, payroll **D17–D18** + **D26**, reputation **D19** + **D28**, training **D20–D21** + **D38–D43** (catalog, completions, log form, **`117`** PDF RLS, **`118`** inservice tables + **hub / new session / inservice→completions**), referrals **D22** + **D27**, dietary **D23**, transportation **D24–D25**) **shipped** (2026-04-09). Core operational visibility for Phase 6 modules 11, 12, 14, 15, 22, 23 is in repo. **Enhanced backlog (D44+):** [TRACK-D-ENHANCED-BACKLOG-PLAN.md](./TRACK-D-ENHANCED-BACKLOG-PLAN.md). Shipped history: [TRACK-D-PHASE6-PASS.md](./TRACK-D-PHASE6-PASS.md). Run `segment:gates` per segment.
5. **Track E** — New DDL starting at migration **`119`** only after specs exist and Tracks A–D are appropriately satisfied for your risk tolerance.
