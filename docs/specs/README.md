...
2. **Remote DB migration parity** — Repo sequence **`001`–`119`** (`npm run migrations:check`). Confirm the target project matches with `supabase migration list` after migration **`119`** is applied (`supabase db push`); record and dates: [PHASE1-ENV-CONFIRMATION.md](./PHASE1-ENV-CONFIRMATION.md). Re-run after any migration-adding PR.
