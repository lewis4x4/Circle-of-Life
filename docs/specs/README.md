...
**September 9 primary delivery:** [Facility Operations catalog — COL-132](27-facility-operations-catalog.md) extends OCE for the shared facility application, Homewood first. Use its migration/integration note and the [COL-18 baseline](../facility-operations/COL-18-BASELINE.md) instead of the dated migration counts below. [Current authority — COL-133](27-facility-operations-authority.md) defines explicit current site/subject/recorder permission and its legacy compatibility boundaries. [Versioned applicability, evidence rules and local procedures — COL-135](27-facility-operations-applicability.md) publishes effective-dated central requirement versions and facility configurations on that catalog. [Recurrence and due-date evaluator — COL-137](27-facility-operations-evaluator.md) is the single evaluator behind the scheduler, task views and next-due previews; an unknown schedule yields no due judgment. [Subject-scoped occurrences — COL-139](27-facility-operations-occurrences.md) gives every generated occurrence a database-owned activity + site + subject + period identity with pinned governing versions, explicit subject bindings, convergent concurrent generation, service reconciliation of retired subjects, and audited manual/early/late association. [Execution receipts — COL-142](27-facility-operations-receipts.md) records work as immutable, idempotent receipts with a server recorder and recorded-at, a distinct performed-at, explicit late/on-behalf entry, validated values, truthful missing-evidence and awaiting-verification states, and a minimal issue identity. [Issue lifecycle — COL-144](27-facility-operations-issues.md) adds explicit owner and backup assignment with acceptance and cover, waiting with follow-up, resolve and reopen as immutable audited events under current authority and an expected revision, with a backlog projection that keeps unassigned, waiting and uncovered work visible; problem status never touches performance. [Verified evidence — COL-143](27-facility-operations-evidence.md) (source portion; hosted Storage proof still open) gives every piece of evidence an immutable classified identity with an owned object path, forward-only prepare/upload/finalize/fail states, and appended satisfaction of the same performance once the required evidence is finalized, with the client-declared MD5 verified against the Storage-computed eTag before anything finalizes. [Corrections, reversals and bound reviews — COL-145](27-facility-operations-corrections.md) appends corrections as new receipts that supersede the current one under an expected revision and reason, carries evidence across the chain, reverses recorded work back to unrecorded without erasing history, binds each review to the exact receipt it reviewed, and proves every legacy write path is refused for managed occurrences. [Interrupted-save recovery — COL-146](27-facility-operations-recovery.md) keeps the exact arguments of a pending command as a server-owned draft under the actor's own identity, reconciles a lost answer by request key, resumes the stored arguments through the idempotent command, never reports saved without a server record, and hides every draft from any other person on a shared device. [Staff site workspace — COL-148](27-facility-operations-workspace.md) brings Today, Upcoming and History into both administrator entry points, with conditional inputs/evidence, current-person receipts, interrupted-save recovery, corrections and stable list position. [Source links — COL-147](27-facility-operations-source-links.md) lets a final domain record satisfy exactly its occurrence once through an allowlisted reader consulted live at delivery, converges replays and concurrent deliveries on one ledger row, refuses drafts, stale versions and wrong scope with a recorded reason, appends corrections and voids as 344 chain history, and keeps pending reconciliation visible; no adapter is registered there. [Drill and generator sources — COL-154](27-facility-operations-drill-generator-sources.md) registers the first two adapters: `drill_log` gains an explicit finality lifecycle (every existing row is a draft until a person finalizes it) and a new `asset_observations` table records staff-observed generator tests, carbon-monoxide checks and extinguisher currency checks against a named asset, refusing an automatic self-test or a photo alone by name; finalize, correct and void commands deliver through 346 in the same transaction, review activities are never allowlisted, and no Homewood rule, day, time or count is set. [Dietary, facility-services and general-admin sources — COL-159](27-facility-operations-dietary-admin-sources.md) completes the AL coverage pass: every remaining item is a registered source path (AED checks on the widened `asset_observations`; site and asset services on a new `facility_service_records` table whose stated next-due date and certificate reference are read-only and which never writes an asset, profile date, document or ticket; meal-level substitutions, dietitian menu approval and the emergency food check on a new `dietary_records` table with no resident), a human recording path with its question named, or a review activity that is never allowlisted; nothing is sent or published. [Task help and handover — COL-153](27-facility-operations-help-handover.md) adds optional activity guidance and explicit, effective-dated owner/backup acceptance while preserving task rules and prior duty history. The isolated staging integration preserves main through339, applies Finance340–346 before HFO347–362, and adds export-scope reconciliation363; hosted application is separately evidenced.

**Unified forward roadmap (all tracks + proposed work):** [UNIFIED-ROADMAP.md](./UNIFIED-ROADMAP.md) — includes **Track F (Employee Workspace & Office Suite)**. This isolated staging integration has **366 migration files**, numbered **001–363**, with current main through339 preserved and Finance-before-HFO ordering. See [pinned integration manifest](../facility-operations/col217-evidence/integration-manifest.json); repository presence does not establish hosted application.
...
| Order | Spec file | Module | Migration range | Audit / build notes |
|-------|-----------|--------|-----------------|---------------------|
| 23 | `12-training-competency.md` | Training and Competency | `086`–`087`, `115`–`117` | ✅ **Spec written.** `competency_demonstrations`; `training_programs` + `staff_training_completions` (`116`); completion PDF storage RLS (`117`). |
...
| `417`–`432` | `25A-smart-rounding-cadence-and-watchlist.md` | Smart Rounding rebuild: shift model and versioned cadence (`417`), chip vocabulary and the composing submit RPC (`418`), Monitoring Orders (`419`), versioned escalation policy (`420`), the projected compliance read and facility inheritance (`421`, `426`, `427`), review fixes and the module authority probe (`422`–`424`), Watchlist signals (`425`), configuration templates and jurisdiction floors (`428`), the seven configuration RPCs (`429`), escalation drain and scope transfers (`430`), the board's display thresholds (`431`), the Monitoring Order interval presets and the grace formula divisor (`432`). **Applied to Haven HFO Staging through the old `424` at the Part 8 handoff; `431` and `432` are not applied anywhere; not applied to production at all.** **These numbers moved once already.** The module was authored at `414`–`429` and renumbered to `417`–`432` when `origin/main`'s own `414`–`416` merged in mid-review; re-run `npm run migrations:check` and `npm run migrations:check:claims` immediately before any push. Supersedes the observation cadence set by `219` and `310`. |
...
**Enhanced backlog (D41+):** [TRACK-D-ENHANCED-BACKLOG-PLAN.md](./TRACK-D-ENHANCED-BACKLOG-PLAN.md) — option comparison (**D12–D40** shipped 2026-04-09 per [TRACK-D-PHASE6-PASS.md](./TRACK-D-PHASE6-PASS.md)).
...
| 31A | `25A-smart-rounding-cadence-and-watchlist.md` | Smart Rounding: cadence, Monitoring Orders, Watchlist, Settings | `417`–`432` | 🟡 **Built on branch `blewis/smart-rounding-cadence-watchlist`, Parts 1–8 complete. Applied to Haven HFO Staging through the old `424` as of the Part 8 handoff; the two Part 8 review migrations (`431`, `432`) are applied nowhere; NOT applied to production at all.** Rebuilds the observation model on versioned per-facility configuration: six windows, chip-composed capture, Monitoring Orders in place of the watch approval queue, a versioned escalation ladder, the Watchlist, a five-tab shell and an administrator settings surface. Spec section 11 open items 1–8 remain open. |
