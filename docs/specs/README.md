...
| Phases 3–6 — live proof / operational hardening | **Incomplete** until Track A closes; **Tracks B–C engineering complete** (see sections below); **Track D** Core **D1–D10** + Enhanced **D12–D56** shipped (see [TRACK-D-PHASE6-PASS.md](./TRACK-D-PHASE6-PASS.md)) — further Enhanced backlog per plan | Same tables + [TRACK-D-PHASE6-PASS.md](./TRACK-D-PHASE6-PASS.md) |
...
4. **Track D** — **Segments D1–D10** (2026-04-09) + Enhanced **D12–D56** (incl. Module 11 hubs **D29–D33** + **D35–D37**, **D34** shared CSV helpers, payroll **D17–D18** + **D26**, reputation **D19** + **D28** + **D44**–**D49** (Google OAuth + imports + cron + Yelp Fusion + **`POST /api/reputation/replies/[id]/post-google`** + **`POST /api/reputation/replies/[id]/post-yelp`**), training **D20–D21** + **D38–D43** (catalog, completions, log form, **`117`** PDF RLS, **`118`** inservice tables + **hub / new session / inservice→completions**), referrals **D22** + **D27**, dietary **D23** + **D50**–**D53** (`/admin/dietary/clinical-review` med–diet **hints** + **IDDSI labels in callouts**) + **D54**–**D55** (`next.config.ts` **`/<segment>` → `/admin/<segment>`** redirects for route-group hubs), transportation **D24–D25** + **D56** (`/admin/transportation/calendar` **month** view)) **shipped** (2026-04-09). Core operational visibility for Phase 6 modules 11, 12, 14, 15, 22, 23 is in repo. **Enhanced backlog (D57+):** [TRACK-D-ENHANCED-BACKLOG-PLAN.md](./TRACK-D-ENHANCED-BACKLOG-PLAN.md). Shipped history: [TRACK-D-PHASE6-PASS.md](./TRACK-D-PHASE6-PASS.md). Run `segment:gates` per segment.
