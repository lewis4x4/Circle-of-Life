...
**Unified forward roadmap (all tracks + proposed work):** [UNIFIED-ROADMAP.md](./UNIFIED-ROADMAP.md) — includes **Track F (Employee Workspace & Office Suite)**. Remote tracking remains through **`316`**; migrations **`317`–`318`** are local only. Next free **`319`**.
...
**Repo migrations:** **`001`–`318`** (317–318 not deployed) — verify with `npm run migrations:check` and `npm run migrations:verify:pg` before release. Next free file: **`319`**.
...
**Next free migration number:** **`319`** — use for all new DDL after updating this README and the relevant spec.
...
5. **Track E / later DDL** — New files start at migration **`319`** only after specs exist and Tracks A–D are appropriately satisfied for your risk tolerance.
