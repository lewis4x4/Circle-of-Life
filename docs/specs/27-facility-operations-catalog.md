...
The subject registry enforces exactly one typed domain reference, real foreign keys and matching organization/facility at creation. It locks the referenced rows during validation. References preserve site-at-registration history; they are **not access grants** and must not be used as current authority after a subject transfer or termination. COL-133 must revalidate native subject permissions and scope on every future command/read. The registry has no client access or public command in this segment, preventing broad legacy task views from exposing restricted subject identities prematurely.

## Authorization and interfaces

RLS is enabled before seed. Catalog reads use current database actor/organization and facility scope. Legacy activity visibility additionally requires an existing visible, nondeleted template. Source and activity rows carry no resident/employee values. Direct authenticated catalog writes are denied. Private trigger functions pin their search path and are not public callable commands; source mappings and identity references are immutable.

Existing template and task reads add `activity_id`. Older client response types accept its absence during transition; server shaping returns null for unknown template-free work. Template mutation payloads cannot choose or rewrite stable IDs. No new staff screen, completion command, history pagination or operating rule is claimed by this catalog segment.
