...
| **11** | ~~**Staff roster CSV** on `/admin/staff`~~ (D29); ~~**staff roster CSV hub filter scope**~~ (D80); ~~**Certifications CSV** on `/admin/certifications`~~ (D30); ~~**Time records CSV** on `/admin/time-records`~~ (D31); ~~**time records CSV approval scope**~~ (D79); ~~**Staffing snapshots CSV** on `/admin/staffing`~~ (D32); ~~**Schedule weeks CSV** on `/admin/schedules`~~ (D33); ~~**Schedule week detail + shift assignments CSV**~~ (D35); ~~**Shift swap requests hub + CSV**~~ (D36); ~~**Shift swap approve/deny**~~ (D37); ~~**Bulk approve** pending time punches~~ (D60); full scheduling builder UX depth |

**Authoritative README narrative:** [README.md](./README.md) — section **Track D — Phase 6 completion pass**.

**Next segment:** Record as **D81+** in this file when a new bounded slice ships; use migration **`120+`** only when DDL is required ([README.md](./README.md) next free migration).
