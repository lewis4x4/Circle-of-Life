...
- **Tries left is shown on the kiosk name path only (migration 552).** A tapped name already names the person, so "Wrong PIN. 3 tries left." confirms nothing; `haven.timeclock_resolve` passes `tries_left` on only when `p_staff_id` is set, and the employee-number and badge paths below are unchanged.
