...
**Shipped (Track D74):** Same hub — **Search** (client-side) on loaded batches: **period** dates, **provider**, **notes**, **id**, **status**; composes with **status** filter; summary count matches visible rows; **CSV** unchanged (**status** scope only).
