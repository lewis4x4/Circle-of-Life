...
- `/admin`
- `/admin/search` (resident-name lexical index search; live resident scope via migration `332`. Staff, vendor and incident records are opened through their existing modules until their indexing contracts are implemented. Empty results here describe residents only.)
...
- `/admin/executive/standup/history` (archive + workbook import runbook + import job status)
- `/admin/executive/standup/[week]`
- `/admin/executive/standup/[week]/board` (board packet preview / print / saved-report entry point)
...
- `/admin/transportation/mileage-approvals` (`mileage_logs` approval queue — Track D15; **mileage CSV scope** — D78)
