...
**Last run (repo/CLI):** 2026-04-10 — `npm run migrations:check`: **118** migration files **001–118** in [`supabase/migrations/`](../../supabase/migrations/) — see [README.md](./README.md) (next migration **119**).

**Last run (remote):** Re-run `supabase migration list` after **`118`** (`supabase db push`); prior **PASS** was **001–117** on **`manfqmasfqppukpobpld`**. Re-run after each migration-adding PR before release.
...
**Local repo (file count / sequence):** **PASS** — `migrations:check` reports **001–118** (2026-04-10).

**Remote (target Supabase project):** Apply **`118`** then confirm parity **001–118** (`manfqmasfqppukpobpld`). If a pull adds `119+`, run `supabase db push` (or CI) until this command shows parity again.
...
| PH1-P02 | **PASS (2026-04-10)** | `supabase migration list` — **001–118** local/remote parity after **`118`** pushed (agent-verified); re-verify after future migration PRs |
