...
**Last run (repo/CLI):** 2026-04-10 — `npm run migrations:check`: **116** migration files **001–116** in [`supabase/migrations/`](../../supabase/migrations/) — see [README.md](./README.md) (next migration **117**).

**Last run (remote):** **PASS (2026-04-10)** — `supabase migration list` on linked project **`manfqmasfqppukpobpld`** shows **Local** and **Remote** columns identical for **001** through **116** (no drift rows). Re-run after each migration-adding PR before release.
...
**Local repo (file count / sequence):** **PASS** — `migrations:check` reports **001–116** (2026-04-10).

**Remote (target Supabase project):** **PASS** — `supabase migration list` Local/Remote aligned **001–116** on **2026-04-10** (project `manfqmasfqppukpobpld`). If a pull adds `117+`, run `supabase db push` (or CI) until this command shows parity again.
...
| PH1-P02 | **PASS (2026-04-10)** | `supabase migration list` — **001–116** local/remote parity on `manfqmasfqppukpobpld` (agent-verified); re-verify after future migration PRs |
