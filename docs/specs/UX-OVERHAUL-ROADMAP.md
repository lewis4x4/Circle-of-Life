...
- **Build**: Implement a Kanban or State-Machine layout (`New` -> `Investigating` -> `Regulatory Review` -> `Closed`). Add explicit countdown ribbons (e.g., `<Badge variant="destructive">2 Hours to DOH Deadline</Badge>`).
