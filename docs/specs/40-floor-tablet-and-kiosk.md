...
Migration numbers for this build are 483 (floor tablets and kiosk) and 484 (rounding owner from punches). Numbers move quickly on this repo: 481 and 482 are held by open PRs #808 and #811 and listed in ALLOWED_GAPS in scripts/check-migration-order.mjs until those files merge. Recheck `npm run migrations:next` before merging.
