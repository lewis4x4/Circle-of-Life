...
Migration numbers for this build are 481 (floor tablets and kiosk) and 482 (rounding owner from punches). 478 to 480 were applied to production from other branches before their files reached main (PR #788, PR #789, and an unpushed COL-668 follow-up); they are listed in ALLOWED_GAPS in scripts/check-migration-order.mjs until those files merge.
...
4. **Resident, Tier 2** (`04`): header with status, today's checks, watch and follow-ups, know-before-you-go-in (the same instruction fields the caregiver resident hub shows), actions Chart this check, Something happened, Full record (Tier 3, existing resident timeline).
5. **Chart a check** (`05`): the rounding completion payload as chips: quick status (`awake, asleep, calm, agitated, confused, distressed, not_found, refused`), location (from `observation_vocab`), helped with (`toileting_assisted, hydration_offered, repositioned`), anything wrong (`pain_concern, breathing_concern, skin_concern_observed, fall_hazard_observed, refused_assistance`), late reason required when over, note optional. Save goes through the existing completion path and the offline queue.
