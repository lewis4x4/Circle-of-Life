...
| **H** | Smart Rounding (Module 25A: six-window versioned cadence, chip-composed capture, Monitoring Orders, versioned escalation ladder, Watchlist, five-tab shell, administrator settings) | **Parts 1–8 built** on branch `blewis/smart-rounding-cadence-watchlist`; migrations `417`–`432`; **partly applied to Haven HFO Staging; `431` and `432` applied nowhere; not applied to production**; spec §11 open items 1–8 still open and owner decision D12 (pool task assignment) is a known functional gap | [25A-smart-rounding-cadence-and-watchlist.md](./25A-smart-rounding-cadence-and-watchlist.md), [HANDOFFS/2026-09-16__smart-rounding-build-notes.md](../../HANDOFFS/2026-09-16__smart-rounding-build-notes.md) |
| **I** | Shared floor tablets and front-door kiosk (Module 40, COL-677; children COL-690 to COL-695: on-clock roster + timeclock PIN unlock, `/floor` app, `/kiosk` staff punch + visitor log, rounding owner from punches, verification, Homewood setup) | In build on branch `blewis/col-677-floor-tablet-kiosk`; migrations `481` and `482`; applied to Haven HFO Staging only for the fidelity run | [40-floor-tablet-and-kiosk.md](./40-floor-tablet-and-kiosk.md), [design contract](../designs/floor-tablet-kiosk/DESIGN.md), [HANDOFFS/2026-09-23__floor-tablet-and-kiosk](../../HANDOFFS/2026-09-23__floor-tablet-and-kiosk/README.md) |
...
5. **Smart Rounding (Track H) before production:** the module is complete on its branch and live on staging, and nothing about it is in production. Four things gate the promotion, and none of them is code:
   ...
   - **Owner decision on build-note D12.** A shift-change task with no incoming assignment is completable by nobody below `nurse` (now `med_tech`, COL-615), so the 06:00 check the cadence exists for can sit unworkable on the board. The fix is assignment (auto-assign on-duty staff, or an explicit claim that writes a `rescue` row), never a looser assignee guard.
   ...
   - **Staging is behind the branch.** The hosted row level security check ran there once and stopped on a script bug, now fixed; migrations `431` and `432` have to be applied to staging before it runs through, because the board's display thresholds and the Monitoring Order interval presets are columns those two add. Without them the interval picker offers nothing and the board reports a configuration gap for every building.
