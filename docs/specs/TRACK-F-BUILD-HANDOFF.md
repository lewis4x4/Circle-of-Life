...
| F3-8-workspace-search | 2026-06-13 | `test-results/agent-gates/2026-06-14T02-32-31-929Z-F3-8-workspace-search.json` (PASS, `--ui`) | `f9a66472` | pass — no DDL; keyword search across pages/files/cards + local "Ask Grace about my notes" retrieval over RLS-scoped private notes (no external egress); ranking unit-tested |
| F4-4-survey-readiness-binder | 2026-06-14 | `test-results/agent-gates/2026-06-14T02-38-34-593Z-F4-4-survey-readiness-binder.json` (PASS, `--ui`) | `14d6efad` | pass — migration `303` curated AHCA readiness checklist layered over live evidence (facility documents, in-services, drills due, last survey); admin/office RLS, audit + soft-delete |
| F5-1-google-drive-import | 2026-06-14 | `test-results/agent-gates/2026-06-14T02-47-01-786Z-F5-1-google-drive-import.json` (PASS, `--ui`) | `eb10ca21` | pass — migration `304` Drive cutover ledger; manifest (JSON/CSV) → per-file mapping to private/team page or KB → import executor reusing `documents` + `workspace_pages` (no parallel file system); parser unit-tested; live binary byte-pull deferred behind owner Drive OAuth |
| F5-2-cutover | 2026-06-14 | `test-results/agent-gates/2026-06-14T02-52-20-852Z-F5-2-cutover.json` (PASS, `--ui`) | `_next commit on main_` | pass — migration `305` immutable Drive cutover attestation; per-facility countdown + readiness rollup from the F5-1 ledger + senior-admin read-only sign-off; helpers unit-tested. **Track F complete except F4-1 eFax (owner vendor pick).** |
