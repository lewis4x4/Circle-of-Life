...
| F2-2-internal-forms-builder | 2026-06-12 | `test-results/agent-gates/2026-06-12T21-37-10-602Z-F2-2-internal-forms-builder.json` (PASS, `--ui`) | _next commit on main_ | pass — migration `291` admin-built forms (jsonb field schema) + status-tracked submission queue (submitted → in_progress → resolved/rejected) with RLS (staff submit/see own, office roles work queue) + audit (grievance evidence) |
