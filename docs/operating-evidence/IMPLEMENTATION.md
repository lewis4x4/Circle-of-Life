...
## Completed bounded segment: source freshness and reporting qualifications

Separate calculation/entry time from source-as-of. Preserve source references and original snapshot scope; attach explicit unknown operational coverage and partial numeric contributor counts. Use the same qualifications in on-screen values, packet and PDF. Matching complete recorded product calculations may be compared; partial, mixed/manual/legacy-unknown or mismatched scopes must not produce unqualified deltas. Existing numeric formulas are not approved COL worksheet definitions; those remain TBD with Jessica. Do not infer operational coverage from row-fill percentage or today's facility directory for historical reports.


Independent source and presentation review APPROVE. Targeted verification passed 119 tests with one existing opt-in skip. Final strict UI gate PASS: `test-results/agent-gates/2026-09-08T17-14-46-219Z-hcol-source-quality-final.json`, including build, lint, security/audit, 339 migrations and 20 SQL probes. Local loader/draft/manual-save/reload through PostgREST preserved source metadata and original scope. Saved-week component browser verification passed keyboard disclosure, overflow, page-error and axe checks with synthetic auth/data. The actual PDF renderer produced 11 inspected pages without blank/orphan-footer pages or clipping. Final visual verdict 95. Evidence: `SOURCE-QUALITY-REVIEW.json`.

Simplification: one shared quality contract qualifies UI, packet and PDF values and comparisons. Existing numeric formulas, scores and numeric snapshot results remain unchanged. Only supported recorded comparisons receive deltas or rankings; source-as-of remains unknown when no source evidence exists. Jessica-owned worksheet definitions remain TBD. This does not certify operational capture coverage, historical publication cutoff, hosted login or staff acceptance. Recorded-bed and pending-versus-completed event corrections remain the next independent segment.


## Completed bounded segment: recorded inventory and pending events

Bounded HCOL-03/04/07/10/24 source-truth correction. Count explicit available, unoccupied, unblocked and unreserved bed rows; no recorded inventory is unknown, and unclassified open rows do not establish category counts. Separate actual-arrival evidence from admission readiness; exclude completed tours and completed provider activity from pending values. Retain existing provisional draft-admission inclusion and home-health-provider category, without inventing approved operating definitions.

New live calculations carry method v2 and captured metric definitions. Preserve historical labels, units and descriptions where captured; otherwise describe the historical definition as unconfirmed. Different or contradictory methods must not yield comparable deltas. Financial formulas and census counts are unchanged. Independent review and strict segment gate completed.


Independent review APPROVE with zero diagnostics. Targeted core/presentation and affected page/state/wall-clock checks passed 145 tests across 11 files, one existing opt-in performance skip. Strict UI gate PASS: `test-results/agent-gates/2026-09-08T18-21-26-176Z-hcol-inventory-pending-events-final.json`, including build, lint, security/audit, 339 migration files, 20 SQL probes and component-harness design/axe checks. Actual loader through local PostgREST verified inventory and pending-event boundaries. Actual manual-save/reload retained a facility input while an incompatible-unit total became unknown, preserving the total's original unit/section and an explicit review note. Source disclosure passed keyboard, mobile overflow, page-error and axe checks; the PDF remained 11 readable pages with the review note visible. Visual verdict 95. Evidence: `INVENTORY-REVIEW.json`.

Review corrections prevented contradictory definition/method comparisons, contributor definitions replacing total definitions, incompatible-unit rollups, and inaccurate method labels. The first gate was intentionally interrupted during build to incorporate review corrections; the final gate ran on the frozen corrected files. No thresholds or fixtures were weakened.

Compatible financial formulas and census values remain unchanged; snapshot numeric differences are limited to the bed metrics and terminal/pre-admission discharge exclusions. No clinical suitability, completed provider handoff, approved draft inclusion, financial worksheet mapping, transactional historical cutoff, or staff/hosted acceptance is inferred. Manual metric writes and portfolio rebuild remain separate requests; broader atomic manual-save/recovery is still open.


## Completed bounded implementation: accountable referral actions

HCOL-08/22 slice adds one open action per referral episode, explicit owner and optional backup, due date or waiting condition, acceptance tied to recorded terms, evidence-backed completion/replacement, immutable event history and command receipts. Existing referral readers retain read access; existing four writer roles manage work. Converted/lost referrals retain unresolved actions. Merge/delete/scope changes cannot silently orphan open work. Current actor/candidate access is rechecked after lock waits.

The detail panel exposes assignment/acceptance, before/after and replacement history, retained stale drafts and an identifier-only recovery journal. Completion evidence stays bound to its original action/version. Unchanged deadlines retain seconds and DST occurrence; nonexistent new local times are rejected. Pipeline filters expose missing actions, acknowledgment, passed explicit dates and unavailable access, with full scoped action pagination and navigable sixty-row display pages. Server bootstrap data is bound to current actor/organization/role/facility before display.

Independent final review APPROVE, zero diagnostics,40 independent tests passed. Parent targeted suite54/54 passed across8files. Strict gate PASS `test-results/agent-gates/2026-09-08T19-32-39-426Z-hcol-referral-next-action.json`: build, lint, security/audit,341 migrations,21 SQL probes, component-harness design/axe. Seventeen actual concurrent checks passed, including revocation while waiting, competing creates, duplicate receipts and inaccessible-target denial before locks. Actual client/API/browser proof passed lost-response reload recovery, explicit owner/backup acceptance, stale completion, reader restrictions, completion/history and hub filters; no overflow/axe/page errors. Visual95. Evidence: `NEXT-ACTION-REVIEW.json`. Full suite `npm test -- --maxWorkers=4` passed540files/3388tests, two existing skips (167.77s). A non-fatal installed-TypeScript sourcemap warning was retained in the run log; no dependencies or thresholds changed.

Actual PostgREST14.4 exposed deterministic40001 retry behavior that nativeSQL tests did not reveal. New logical conflicts usePT409; forward338 changes exactly three conflict-code sites in the existing import/manual-return functions. Original335/336 remain unchanged; byte comparison confirms no other body changes. ActualHTTP stale import/version/context and conflicting completed review now return409/PT409 in8–23ms.

No financial formula, clinical eligibility, stage/closure vocabulary, staffing cadence, external notification, production deployment or staff acceptance is inferred. Eligibility labels mean access active/unavailable. Archived retrieval after a closed lead is deleted/moved, offline draft persistence and broader HCOL work remain open. The approved upstream login release37d0619d was integrated unchanged before this segment and covered by the combined gate.
