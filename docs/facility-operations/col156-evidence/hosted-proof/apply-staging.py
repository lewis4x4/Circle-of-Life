#!/usr/bin/env python3
"""Apply exactly reviewed368 and its ledger row atomically to verified staging."""
...
def validate_ledger(ledger,migration_paths):
 """Accept the exact reviewed local version set, before or after368 only.

 Timestamp migrations are ordinary members of that set, not numeric successors
 of368. Unknown timestamps, omissions and duplicates still fail closed.
 """
...
def build_apply_query(source,versions):
 ...
 return "BEGIN; SET LOCAL lock_timeout='5s'; SET LOCAL statement_timeout='120s'; LOCK TABLE supabase_migrations.schema_migrations IN EXCLUSIVE MODE; DO $$ BEGIN IF (SELECT count(*) FROM supabase_migrations.schema_migrations)<>"+str(len(versions))+" OR EXISTS(SELECT 1 FROM supabase_migrations.schema_migrations WHERE version='368') OR NOT EXISTS(SELECT 1 FROM supabase_migrations.schema_migrations WHERE version='367') OR (SELECT array_agg(version ORDER BY version) FROM supabase_migrations.schema_migrations) IS DISTINCT FROM "+expected_array+" THEN RAISE EXCEPTION 'Ledger changed'; END IF; END $$;\n"+body+"\nINSERT INTO supabase_migrations.schema_migrations(version,name,statements) VALUES('368','hfo_employee_source_snapshots',ARRAY["+literal(source)+"]); NOTIFY pgrst,'reload schema'; COMMIT;"
...
def main():
 ...
 if '368' in versions:
  stored=r.sql("SELECT statements[1] FROM supabase_migrations.schema_migrations WHERE version='368'")
  require(stored.strip()==source.strip(),'Existing368 ledger does not match reviewed source')
 elif args.action=='apply':
  query=build_apply_query(source,versions)
  r.identify();r.sql(query)
 else:
  print(json.dumps({'target':REF,'pending':'368','mutation':False}));return
 posture=json.loads(r.sql("SELECT json_build_object('invoker',NOT p.prosecdef,'authenticated',has_function_privilege('authenticated',p.oid,'EXECUTE'),'anon_denied',NOT has_function_privilege('anon',p.oid,'EXECUTE'),'service_denied',NOT has_function_privilege('service_role',p.oid,'EXECUTE')) FROM pg_proc p WHERE p.oid=to_regprocedure('public.employee_operation_source_snapshot(uuid,text)')"))
 require(all(posture.values()),'Unexpected RPC posture')
 result={'result':'PASS','target':REF,'sourceSha':r.ready['sourceSha'],'migration_sha256':sha,'project':project,'posture':posture,'at':datetime.datetime.now(datetime.timezone.utc).isoformat(),'scope':'Staging additive368 only; no rule publication or operating acceptance'}
