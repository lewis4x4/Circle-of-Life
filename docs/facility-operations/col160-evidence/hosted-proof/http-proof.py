...
def main():
 ...
 if args.action=='exercise':
  ...
  missing_meeting=checkpoint('http_missing_meeting',lambda:meeting('missing-meeting'));missing_members=[row for row in missing_meeting['meetings'][0]['snapshot']['sites'] if row['status']=='missing'];require(len(missing_members)==1 and missing_members[0]['expectation'] is None,'Missing meeting member must retain literal JSONnull expectation')
  raw_snapshot=request('corporate_deliverable_snapshot',{'p_task':s['task'],'p_start':s['period']['period_start'],'p_end':s['period']['period_end']},rpc=True);require(raw_snapshot['meetings']==missing_meeting['meetings'],'RawRPC and actual app schema disagree on missing-site meeting')
  meeting_replay=meeting('missing-meeting');require(len(meeting_replay['meetings'])==1 and meeting_replay['meetings']==missing_meeting['meetings'],'Same-key meeting retry duplicated or changed capture')
  require(r.sql('SELECT count(*) FROM haven.corporate_deliverable_requests WHERE request_key='+lit(s['run']+'-missing-meeting')+' AND actor_id='+lit(s['users']['owner']))=='1','Meeting request replay duplicated receipt')
  ...
  for label,bad in [('blank','   '),('nbsp',chr(160)),('bom',chr(65279)),('oversize','x'*1001)]:
