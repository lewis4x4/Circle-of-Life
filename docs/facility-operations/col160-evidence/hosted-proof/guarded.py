...
REF='iwcnajanvjvynolltflw'; MIGRATION='371_hfo_corporate_deliverables.sql'
...
class Runtime:
 def __init__(self,ready_path,stage=False):
  ...
  if stage:
   stamp=json.loads((OUT/'staging-readiness.json').read_text());require(stamp['result']=='PASS' and stamp['target']==REF and stamp['sourceSha']==self.ready['sourceSha'] and stamp['migration_sha256']==digest((ROOT/'supabase/migrations'/MIGRATION).read_bytes()),'Reviewed staging371 readback required')
 def verify(self):
...
if __name__=='__main__':
 ...
 if args.action=='manifest':
  ...
  path=pathlib.Path(args.output).resolve();scratch=pathlib.Path.home()/'.hermes/tmp/agent-runs/haven-col160-closeout-01a09c89'
