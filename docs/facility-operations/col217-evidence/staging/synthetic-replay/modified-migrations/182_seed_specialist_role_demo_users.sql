...
BEGIN
  ...
  FROM auth.users
  WHERE lower(email) = 'synthetic-2f1d599888@example.invalid'
  LIMIT 1;
  ...
  IF frontdesk_id IS NULL THEN
    ...
    INSERT INTO auth.users (
      ...
      frontdesk_id, inst,
      'synthetic-2f1d599888@example.invalid', pw, ts,
      jsonb_build_object(
        'provider', 'email',
      ...
      jsonb_build_object(
        'full_name', 'Synthetic administrator_name d9206d98c1',
        'email_verified', true
      ),
  ...
  ELSE
    UPDATE auth.users
    SET
      email = 'synthetic-2f1d599888@example.invalid',
  ...
  INSERT INTO user_profiles (id, organization_id, email, full_name, phone, app_role) VALUES (
    frontdesk_id,
    org,
    'synthetic-2f1d599888@example.invalid',
    'Synthetic administrator_name d9206d98c1',
    '202-555-0199',
    'admin_assistant'
  )
  ...
  FROM auth.users
  WHERE lower(email) = 'synthetic-c06fabbb98@example.invalid'
  LIMIT 1;
  ...
  IF coordinator_id IS NULL THEN
    ...
    INSERT INTO auth.users (
      ...
      coordinator_id, inst,
      'synthetic-c06fabbb98@example.invalid', pw, ts,
      jsonb_build_object(
        'provider', 'email',
        'providers', jsonb_build_array('email'),
      ...
      jsonb_build_object(
        'full_name', 'Synthetic full_name 49c9f3e7e8',
        'email_verified', true
      ),
      'authenticated', 'authenticated',
      ts, ts,
  ...
  ELSE
    UPDATE auth.users
    SET
      email = 'synthetic-c06fabbb98@example.invalid',
      encrypted_password = pw,
      email_confirmed_at = COALESCE(email_confirmed_at, ts),
      raw_app_meta_data = jsonb_build_object(
        'provider', 'email',
  ...
  INSERT INTO user_profiles (id, organization_id, email, full_name, phone, app_role) VALUES (
    coordinator_id,
    org,
    'synthetic-c06fabbb98@example.invalid',
    'Synthetic full_name 49c9f3e7e8',
    '202-555-0148',
    'coordinator'
  )
  ON CONFLICT (id) DO UPDATE SET
  ...
  FROM auth.users
  WHERE lower(email) = 'synthetic-68518a07ae@example.invalid'
  LIMIT 1;
  ...
  IF housekeeper_id IS NULL THEN
    ...
    INSERT INTO auth.users (
      ...
      housekeeper_id, inst,
      'synthetic-68518a07ae@example.invalid', pw, ts,
      jsonb_build_object(
        'provider', 'email',
        'providers', jsonb_build_array('email'),
      ...
      jsonb_build_object(
        'full_name', 'Synthetic full_name 9cbb65f3ae',
        'email_verified', true
      ),
      'authenticated', 'authenticated',
      ts, ts,
  ...
  ELSE
    UPDATE auth.users
    SET
      email = 'synthetic-68518a07ae@example.invalid',
      encrypted_password = pw,
      email_confirmed_at = COALESCE(email_confirmed_at, ts),
      raw_app_meta_data = jsonb_build_object(
        'provider', 'email',
  ...
  INSERT INTO user_profiles (id, organization_id, email, full_name, phone, app_role) VALUES (
    housekeeper_id,
    org,
    'synthetic-68518a07ae@example.invalid',
    'Synthetic full_name 9cbb65f3ae',
    '202-555-0157',
    'housekeeper'
  )
  ON CONFLICT (id) DO UPDATE SET
