#!/usr/bin/env python3
"""Exact COL221 synthetic staging fixture; no production or real resident data."""
...
PRIVATE=pathlib.Path.home()/'.config/haven-staging/col221-fixture.json'
def envfile(p):
...
BASE=env['NEXT_PUBLIC_SUPABASE_URL']; state=json.loads(PRIVATE.read_text()) if PRIVATE.exists() else {'run':'col221-'+secrets.token_hex(6),'target':REF,'entity':str(uuid.uuid4()),'site':str(uuid.uuid4()),'subject':str(uuid.uuid4()),'activity':str(uuid.uuid4())}
assert state['target']==REF and state['run'].startswith('col221-')
def save():
...
if mode=='serve':
 ...
 os.chdir(ROOT);os.execvpe('npm',['npm','run','dev','--','--hostname','127.0.0.1','--port','4341'],e)
elif mode=='setup':
 ...
 stage=json.loads(sql("SELECT json_agg(x) FROM (SELECT version,name FROM supabase_migrations.schema_migrations) x"));local={p.name.split('_',1)[0] for p in (ROOT/'supabase/migrations').glob('*.sql')};assert {x['version'] for x in stage}==local|{'366'} and next(x for x in stage if x['version']=='366')['name']=='hfo_profile_draft_preparation','Unexpected staging ledger';(OUT/'stage-ledger.json').write_text(json.dumps({'local_max_numeric':'365','reviewed_additive_staging_version':'366','ledger':stage},indent=2))
 if 'user' not in state:
  state['email']=state['run']+'@example.invalid';state['password']=secrets.token_urlsafe(30);save()
  state['user']=checkpoint('auth_user',lambda:http('/auth/v1/admin/users',{'email':state['email'],'password':state['password'],'email_confirm':True,'user_metadata':{'full_name':'COL221 Synthetic Operator'}},admin=True))['id'];state.pop('auth_user',None);save()
...
INSERT INTO public.user_profiles(id,email,full_name,app_role,organization_id,is_active) VALUES({lit(state['user'])},{lit(state['email'])},'COL221 Synthetic Operator','owner',{lit(ORG)},true);
