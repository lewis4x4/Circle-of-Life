...
def verify(ready_path):
 r=json.loads(pathlib.Path(ready_path).read_text());require(r['target']==REF and r['appUrl']=='http://127.0.0.1:4342' and pathlib.Path(r['checkout']).resolve()==ROOT,'Wrong proof target/origin')
 require(r['approved_by_parent'] is True and r['scope']=='COL-224 synthetic fixture and browser proof' and r['phase'] in ['proof'],'Explicit parent readiness required')
...
if __name__=='__main__':
 p=argparse.ArgumentParser();p.add_argument('action',choices=['manifest','verify']);p.add_argument('--ready');p.add_argument('--phase',choices=['proof']);args=p.parse_args()
