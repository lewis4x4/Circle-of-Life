#!/usr/bin/env python3
"""COL157 source/readiness guard. 'manifest' is local-only; no network on import."""
...
REF='iwcnajanvjvynolltflw'; MIGRATION='369_hfo_finance_source_snapshots.sql'
...
class Runtime:
 def __init__(self,ready_path,stage=False):
  ...
  self.base=self.env['NEXT_PUBLIC_SUPABASE_URL'];self.app='http://127.0.0.1:4357'
  if stage:
   stamp=json.loads((OUT/'staging-readiness.json').read_text());require(stamp['result']=='PASS' and stamp['target']==REF and stamp['sourceSha']==self.ready['sourceSha'] and stamp['migration_sha256']==digest((ROOT/'supabase/migrations'/MIGRATION).read_bytes()),'Reviewed staging369 readback required')
 def verify(self):
  r=json.loads(self.ready_path.read_text());self.ready=r
  require(r['target']==REF and r['appUrl']=='http://127.0.0.1:4357','Wrong readiness target/origin')
