#!/usr/bin/env python3
"""Exact COL227 synthetic staging fixture; no production or real resident data."""
...
PRIVATE=pathlib.Path.home()/'.config/haven-staging/col227-fixture.json'
def envfile(p):
...
BASE=env['NEXT_PUBLIC_SUPABASE_URL']; state=json.loads(PRIVATE.read_text()) if PRIVATE.exists() else {'run':'col227-'+secrets.token_hex(6),'target':REF,'entity':str(uuid.uuid4()),'site':str(uuid.uuid4()),'subject':str(uuid.uuid4()),'activity':str(uuid.uuid4())}
assert state['target']==REF and state['run'].startswith('col227-')
def save():
...
if mode=='serve':
 ...
 os.chdir(ROOT);os.execvpe('npm',['npm','run','dev','--','--hostname','127.0.0.1','--port','4343'],e)
elif mode=='setup':
 ...
 if 'user' not in state:
  ...
  state['user']=checkpoint('auth_user',lambda:http('/auth/v1/admin/users',{'email':state['email'],'password':state['password'],'email_confirm':True,'user_metadata':{'full_name':'COL227 Synthetic Operator'}},admin=True))['id'];state.pop('auth_user',None);save()
...
INSERT INTO public.user_profiles(id,email,full_name,app_role,organization_id,is_active) VALUES({lit(state['user'])},{lit(state['email'])},'COL227 Synthetic Operator','owner',{lit(ORG)},true);
