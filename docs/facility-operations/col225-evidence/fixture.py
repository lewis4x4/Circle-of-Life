#!/usr/bin/env python3
"""Exact COL225 synthetic staging fixture; no production or real resident data."""
...
PRIVATE=pathlib.Path.home()/'.config/haven-staging/col225-fixture.json'
def envfile(p):
...
def legacy_id():
 value=list(str(uuid.uuid4()));value[14]='0';value[19]='0';return ''.join(value)
BASE=env['NEXT_PUBLIC_SUPABASE_URL']; state=json.loads(PRIVATE.read_text()) if PRIVATE.exists() else {'run':'col225-'+secrets.token_hex(6),'target':REF,'entity':legacy_id(),'site':legacy_id(),'siteb':legacy_id(),'subject':legacy_id(),'activity':legacy_id()}
assert state['target']==REF and state['run'].startswith('col225-')
def save():
...
if mode=='serve':
 ...
 os.chdir(ROOT);os.execvpe('npm',['npm','run','dev','--','--hostname','127.0.0.1','--port','4325'],e)
elif mode=='setup':
 ...
 if 'user' not in state:
  ...
  state['user']=checkpoint('auth_user',lambda:http('/auth/v1/admin/users',{'email':state['email'],'password':state['password'],'email_confirm':True,'user_metadata':{'full_name':'COL225 Synthetic Operator'}},admin=True))['id'];state.pop('auth_user',None);save()
...
INSERT INTO public.facilities(id,entity_id,organization_id,name,address_line_1,city,zip,total_licensed_beds,timezone) VALUES({lit(state['siteb'])},{lit(state['entity'])},{lit(ORG)},{lit(state['run']+' Site B')},'Synthetic','Test','00000',1,'America/New_York');
...
INSERT INTO public.user_profiles(id,email,full_name,app_role,organization_id,is_active) VALUES({lit(state['user'])},{lit(state['email'])},'COL225 Synthetic Operator','owner',{lit(ORG)},true);
...
COMMIT;"""
 ...
 sql(f"BEGIN; UPDATE public.user_facility_access SET revoked_at=clock_timestamp() WHERE user_id={lit(state['user'])}; UPDATE public.user_profiles SET is_active=false WHERE id={lit(state['user'])}; UPDATE public.facilities SET deleted_at=clock_timestamp() WHERE id IN({lit(state['site'])},{lit(state['siteb'])}); COMMIT;")
