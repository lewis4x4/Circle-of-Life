# COL-143 — scoped evidence delivery

**Hosted synthetic proof completed and independently technically reviewed.** Tested source: `b3e1d819874c942afb737f46eaa632ba54de247c`; staging project `iwcnajanvjvynolltflw`. Administrator, assistant, late-entry and required-checksum-failure/retry flows completed the same original performance, with real Storage ownership/checksums and corporate byte verification. Negative authorization, stale update, immutable-finalized-file and revocation cases passed.

See [hosted RESULT](col143-evidence/hosted-proof/RESULT.md) and [independent Codex technical review](col143-evidence/hosted-proof/INDEPENDENT-REVIEW.md). All164 records are redacted; exactly7ownedobjects were removed and4fixtureactors disabled after read-only UI smoke. Immutable history remains.

This is source delivery plus real hosted synthetic backend proof using a local app. Production, public-app deployment, clinical, staff and operating-cycle acceptance remain separate. UI followups COL-221 through COL-224 remain open. No human approval is inferred from the independent Codex review.

## Earlier source-only record — retained for history

The status and provisional filenames below describe the earlier source-only phase. Its hosted-proof blocker is superseded by the completed records above.
...
0. **Trust-anchor precondition.** In the hosted project confirm that `storage` is absent from the exposed API schemas (Settings → API → Exposed schemas; expected `public, graphql_public`). If `storage` is exposed, stop: a client could write `storage.objects` metadata directly and the eTag the database verifies against would no longer be a value only the Storage service wrote. Retain a screenshot or the settings API response.
...
3. Compute the file's MD5 in the browser (`src/lib/operations/md5.ts`, `md5OfBlob`) and `POST /api/admin/operations/evidence` for the photo rule with `md5` in the payload; upload the file with the returned signed URL (HTTP 200 from Storage).
4. **eTag confirmation (checksum amendment §6).** After the upload, `HEAD` the object through a signed download URL and read `storage.objects.metadata->>'eTag'` for the path: confirm it equals the MD5 of the uploaded bytes (quotes and any `W/` prefix aside) and that `owner` is the uploader's uuid; retain the response headers (no bytes). If the eTag is not the plain MD5 (for example a multipart `<md5>-N` fingerprint or another format), do not proceed to finalize: record the observed format here and switch to the fallback verifier (an app-tier verifier that streams the object through the session's signed URL and presents the hash under a database-held secret) before any hosted evidence is finalized. Locally the command reports such an eTag as `checksum_unverifiable` and refuses finalization.
5. `POST …/evidence/[id]/uploaded` (expect `evidence_outcome: "uploaded"` with `checksum_verified`, `checksum_method: "storage_etag_md5"` and `object_version` set); `POST …/evidence/[id]/finalize` with the receipt revision; confirm the occurrence is `completed` (or `awaiting_verification`) and the receipt's current evidence status is `complete`.
6. As a corporate user with the site grant, `GET …/evidence/[id]/download` and fetch the signed URL (HTTP 200; the object's size and type match). Retain the request and response logs (no object bytes) under `col143-evidence/hosted-proof/`.

Differences between the local stand-in and hosted Storage that the proof must confirm (recorded by the SQL review): the hosted Storage API writes the uuid `owner` column only when the token's owner is a uuid (and always writes the text `owner_id`), so the proof must show `owner` non-null for a signed-upload-URL upload, otherwise nothing finalizes and the command must be switched to `owner_id`; the metadata keys `size`, `mimetype` and `eTag` match the Storage API and the `version` column exists on hosted `storage.objects` (the local stub now carries it); the eTag of a signed-URL upload of at most twenty mebibytes must be the plain quoted MD5 of the bytes for the database verification to hold (step 4 above; otherwise the fallback verifier); the signed upload URL is authorized against the insert policy at signing time while the PUT itself runs as the Storage service, so a row failed between signing and upload leaves a harmless orphan object that finalize refuses; the `storage.objects` policies rely on the same hosted privileges as migration 335.
