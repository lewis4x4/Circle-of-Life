...
def main():
 ...
 ledger=json.loads(r.sql("SELECT json_agg(x ORDER BY version) FROM (SELECT version,name FROM supabase_migrations.schema_migrations) x"));versions={x['version'] for x in ledger}
 require('365' in versions and not any(v.isdigit() and int(v)>366 for v in versions),'Unexpected ledger')
 if '366' in versions:
