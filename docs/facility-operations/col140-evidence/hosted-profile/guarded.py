#!/usr/bin/env python3
"""COL140 source/readiness guard. 'manifest' is local-only; no network on import."""
...
REF='iwcnajanvjvynolltflw'; MIGRATION='366_hfo_profile_draft_preparation.sql'
...
def source_manifest():
 ...
 chosen={name for name in names if name.startswith(('src/','supabase/','scripts/','.github/')) or name in ['package.json','package-lock.json','next.config.ts','tsconfig.json','tsconfig.typecheck.json'] or (name.startswith(str(OUT.relative_to(ROOT))+'/') and name.endswith(('.py','.mjs')))}
...
class Runtime:
 def __init__(self,ready_path,stage=False):
  ...
  self.base=self.env['NEXT_PUBLIC_SUPABASE_URL'];self.app='http://127.0.0.1:4340'
  if stage:
   stamp=json.loads((OUT/'staging-readiness.json').read_text());require(stamp['result']=='PASS' and stamp['target']==REF and stamp['sourceSha']==self.ready['sourceSha'] and stamp['migration_sha256']==digest((ROOT/'supabase/migrations'/MIGRATION).read_bytes()),'Reviewed staging366 readback required')
 def verify(self):
  r=json.loads(self.ready_path.read_text());self.ready=r
  require(r['target']==REF and r['appUrl']=='http://127.0.0.1:4340','Wrong readiness target/origin')
  ...
  for key in ['gate','review']:
   ...
   if key=='gate': require(artifact['sourceSha']==r['sourceSha'] and value['verdict']=='PASS' and any(c.get('required') for c in value['checks']) and all(c['status']=='passed' for c in value['checks'] if c.get('required')),'Required exact-source gates must pass')
 ...
 def sql(self,query):
  self.verify();env={k:v for k,v in os.environ.items() if not k.startswith('PG')};env.update(self.env);env['PGPASSWORD']=self.env['SUPABASE_DB_PASSWORD']
...
if __name__=='__main__':
 parser=argparse.ArgumentParser(description=__doc__);parser.add_argument('action',choices=['manifest','verify']);parser.add_argument('--ready');args=parser.parse_args()
 if args.action=='manifest':
  data=source_manifest();(OUT/'source-manifest.json').write_text(json.dumps(data,indent=2)+'\n');print(json.dumps({'sourceSha':data['sourceSha'],'file_count':len(data['files']),'path':str(OUT/'source-manifest.json')}))
