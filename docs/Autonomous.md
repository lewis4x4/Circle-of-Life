...
| Field | Value |
|-------|--------|
| **BOOT** | `AGENTS.md`, `CODEX.md`, Next.js 16.2.6 instant-navigation / prefetching / streaming / loading / authentication guides, live Supabase migration parity, and authenticated production-build navigation timing. |
...
| **BUILD** | Added destination-local admin loading feedback and palette prefetch; streamed Command Center, Finance, Residents, Staff, and Schedules behind immediate shells; consolidated browser auth and request-scoped server auth; changed proxy authorization to cached ES256 claim verification; made role-home navigation direct; replaced Command Center fan-out with the governed, RLS-preserving `admin_command_center_projection` RPC (migrations `308`–`309`); enabled privacy-scrubbed sampled Sentry tracing; rebuilt the navigation timing probe for cold/warm click metrics. |
| **Evidence** | Enforced local production timing across Residents, Staff, Schedules, and Finance: shell p95 **3.3 ms**, cold content p95 **1762.3 ms**, warm content p95 **422.1 ms**, zero budget failures. Authenticated live RPC returned one valid projection payload; migrations `308`–`309` deployed. Focused tests **5/5**, TypeScript, lint, build (**407/407** pages), gitleaks, stress, design review, and axe passed. Docker replay was skipped after local image acquisition failed; live apply + authenticated transaction/RPC verification covered these migrations. Repository-wide npm advisories are recorded as a baseline advisory because remediation requires an out-of-scope Next/dependency upgrade. |
...
| **Gate artifact** | `test-results/agent-gates/2026-08-14T01-15-50-044Z-NAV-PERF-2026-08-13.json` |
| **Deferred** | Track A owner depth UAT and Pro/BAA/PITR remain human gates. Dependency advisory remediation is a separate framework-upgrade segment. |
