...
## CI entity-request synchronization follow-up

GitHub run 35468946167 found a separate inherited executive-facility test race: the facility heading resolved before its separate entity request, but the test queried the entity link synchronously. It now awaits that link, preserving the destination assertion. All 20 focused tests pass; [required follow-up gate PASS](../test-results/agent-gates/2026-09-19T21-06-24-222Z-SMART-ROUNDING-CI-ENTITY-20260919.json). The other 6,357 tests passed in the failed run. Earlier run 35468464078 completed the entire core workflow successfully, including browser regressions. Final-head CI remains mandatory. Mission alignment: **pass**, accurate asynchronous evidence without changing product behavior.
